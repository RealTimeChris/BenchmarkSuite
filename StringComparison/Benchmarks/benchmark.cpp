...
template<typename CharT> double findmax_fastfloat64(std::vector<std::basic_string<CharT>>& s) {
	double answer = 0;
	double x	  = 0;
	for (auto& st: s) {
...
template<typename CharT> double findmax_fastfloat32(std::vector<std::basic_string<CharT>>& s) {
	float answer = 0;
	float x		 = 0;
	for (auto& st: s) {
		auto [p, ec] = fast_float::from_chars(st.data(), st.data() + st.size(), x);
...
#if defined(USING_COUNTERS
template<class T, class CharT> std::vector<event_count> time_it_ns(std::vector<std::basic_string<CharT>>& lines, T const& function, uint64_t repeat) {
...
void pretty_print(double volume, uint64_t number_of_floats, std::string name, std::vector<event_count> events) {
	...
	printf("%-40s: %8.2f MB/s (+/- %.1f %%) ", name.data(), volumeMB * 1000000000 / min_ns, (average_ns - min_ns) * 100.0 / average_ns);
	printf("%8.2f Mfloat/s  ", number_of_floats * 1000 / min_ns);
	if (instructions_min > 0) {
		printf(" %8.2f i/B %8.2f i/f (+/- %.1f %%) ", instructions_min / volume, instructions_min / number_of_floats,
			(instructions_avg - instructions_min) * 100.0 / instructions_avg);

		printf(" %8.2f c/B %8.2f c/f (+/- %.1f %%) ", cycles_min / volume, cycles_min / number_of_floats, (cycles_avg - cycles_min) * 100.0 / cycles_avg);
...
#else
template<class T, class CharT> std::pair<double, double> time_it_ns(std::vector<std::basic_string<CharT>>& lines, T const& function, uint64_t repeat) {
...
void pretty_print(double volume, uint64_t number_of_floats, std::string name, std::pair<double, double> result) {
	double volumeMB = volume / (1024. * 1024.);
	printf("%-40s: %8.2f MB/s (+/- %.1f %%) ", name.data(), volumeMB * 1000000000 / result.first, (result.second - result.first) * 100.0 / result.second);
...
void process(std::vector<std::string>& lines, uint64_t volume) {
	uint64_t repeat	= 100;
	double volumeMB = volume / (1024. * 1024.);
