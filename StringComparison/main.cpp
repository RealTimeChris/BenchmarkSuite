...
JSONIFIER_ALWAYS_INLINE uint64_t parse_3_chars(const char* string) noexcept {
	uint64_t value{ *std::bit_cast<uint16_t*, const char*>(string) | static_cast<uint64_t>(string[2]) << 16 };
	value = ((value & 0x00000f00) >> 8) + (value & 0x000f000f) * 10;
	return ((((value & 0x00ff0000) >> 16) + (value & 0x000000ff) * 100)) / 10;
}

JSONIFIER_ALWAYS_INLINE uint64_t parse_4_chars(const char* string) noexcept {
	std::uint64_t value;
	std::memcpy(&value, string, sizeof(value));
	std::uint64_t lower_digits = (value & 0x0f000f00) >> 8;
	std::uint64_t upper_digits = (value & 0x000f000f) * 10;
...
JSONIFIER_ALWAYS_INLINE uint64_t parse_5_chars(const char* string) noexcept {
	uint64_t value{ *std::bit_cast<uint64_t*, const char*>(string) | static_cast<uint64_t>(string[4]) << 32 };
...
JSONIFIER_ALWAYS_INLINE uint64_t parse_6_chars(const char* string) noexcept {
	uint64_t value{ *std::bit_cast<uint64_t*, const char*>(string) | static_cast<uint64_t>(string[4]) << 32 | static_cast<uint64_t>(string[5]) << 40 };
	value = ((value & 0x0f000f000f000f00) >> 8) + (value & 0x000f000f000f000f) * 10;
	value = ((value & 0x00ff000000ff0000) >> 16) + (value & 0x000000ff000000ff) * 100;
...
JSONIFIER_ALWAYS_INLINE constexpr uint64_t numbits(uint64_t x) noexcept {
...
template<jsonifier::concepts::float_type value_type, typename char_type> JSONIFIER_MAYBE_ALWAYS_INLINE char_type* toChars(char_type* buf, value_type val) noexcept {
	...
	using raw = std::conditional_t<std::is_same_v<float, value_type>, uint64_t, uint64_t>;

	raw rawVal;
	std::memcpy(&rawVal, &val, sizeof(value_type));

	constexpr bool isFloat				   = std::is_same_v<float, value_type>;
	static constexpr uint64_t exponentBits = numbits(std::numeric_limits<value_type>::max_exponent - std::numeric_limits<value_type>::min_exponent + 1);
	bool sign							   = (rawVal >> (sizeof(value_type) * 8 - 1));
	uint64_t expRaw						   = rawVal << 1 >> (sizeof(raw) * 8 - exponentBits);

	if JSONIFIER_UNLIKELY ((expRaw == (static_cast<uint64_t>(1) << exponentBits) - 1)) {
	...
	*buf = '-';
	...
	if constexpr (isFloat) {
		...
		uint64_t sigDec			= static_cast<uint64_t>(value.significand);
		int64_t expDec			= value.exponent;
		const int64_t numDigits = static_cast<int64_t>(jsonifier_internal::fastDigitCount(sigDec));
		int64_t dotPos			= numDigits + expDec;
		...
		if (-6 < dotPos && dotPos <= 9) {
			if (dotPos <= 0) {
				...
				*buf++ = '.';
				...
				int64_t digitsWritten = static_cast<int64_t>(numEnd - buf);
			...
			expDec += static_cast<int64_t>(end - (buf + 1)) - 1;
			buf[0] = buf[1];
			buf[1] = '.';
			if (end == buf + 2) {
				buf[2] = '0';
...
int32_t main() {
	std::string newerString{ "6495592898994804969" };
	jsonifier_internal::string_literal testLiteral{ "6266621410100944238" };
	...
	std::cout << "CURRENT VALUE: " << jsonifier_internal::integer_parser<int64_t, char>::parseInt(newValue, newIter) << std::endl;
