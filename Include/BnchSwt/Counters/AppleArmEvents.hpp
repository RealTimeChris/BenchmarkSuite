...
namespace bnch_swt {
	...
	struct performance_counters {
		...
		BNCH_SWT_ALWAYS_INLINE performance_counters(uint64_t c, uint64_t b, uint64_t m, uint64_t i) : cycles(c), branches(b), missed_branches(m), instructions(i) {
		}

		BNCH_SWT_ALWAYS_INLINE performance_counters(double c, double b, double m, double i) : cycles(c), branches(b), missed_branches(m), instructions(i) {
		}

		BNCH_SWT_ALWAYS_INLINE performance_counters(double init = 0.0f) : cycles(init), branches(init), missed_branches(init), instructions(init) {
		}
		...
		BNCH_SWT_ALWAYS_INLINE performance_counters& min(const performance_counters& other) {
			...
			missed_branches = other.missed_branches < missed_branches ? other.missed_branches : missed_branches;
	...
	BNCH_SWT_ALWAYS_INLINE performance_counters operator-(const performance_counters& a, const performance_counters& b) {
		return performance_counters(a.cycles - b.cycles, a.branches - b.branches, a.missed_branches - b.missed_branches, a.instructions - b.instructions);
	}
