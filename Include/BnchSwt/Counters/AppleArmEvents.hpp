...
namespace bnch_swt {
	...
	struct performance_counters {
		...
		BNCH_SWT_INLINE performance_counters(uint64_t c, uint64_t b, uint64_t m, uint64_t i) : cycles(c), branches(b), missed_branches(m), instructions(i) {
		}

		BNCH_SWT_INLINE performance_counters(double c, double b, double m, double i) : cycles(c), branches(b), missed_branches(m), instructions(i) {
		}

		BNCH_SWT_INLINE performance_counters(double init = 0.0f) : cycles(init), branches(init), missed_branches(init), instructions(init) {
		}
 ...
 */
	...
	template<typename event_count> struct event_collector_type {
		...
		template<typename function_type, typename... arg_types> BNCH_SWT_INLINE event_count start(function_type&& function, arg_types&&... args) {
			event_count count{};
