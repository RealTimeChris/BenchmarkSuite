...
namespace bnch_swt {
	...
	struct performance_counters {
		...
		BNCH_SWT_ALWAYS_INLINE performance_counters(uint64_t c, uint64_t b, uint64_t m, uint64_t i) : cycles(c), branches(b), missedBranches(m), instructions(i) {
		}

		BNCH_SWT_ALWAYS_INLINE performance_counters(double c, double b, double m, double i) : cycles(c), branches(b), missedBranches(m), instructions(i) {
		}

		BNCH_SWT_ALWAYS_INLINE performance_counters(double init = 0.0f) : cycles(init), branches(init), missedBranches(init), instructions(init) {
		}

		BNCH_SWT_ALWAYS_INLINE performance_counters& operator-=(const performance_counters& other) {
			cycles -= other.cycles;
		...
		BNCH_SWT_ALWAYS_INLINE performance_counters& min(const performance_counters& other) {
			cycles		   = other.cycles < cycles ? other.cycles : cycles;
			branches	   = other.branches < branches ? other.branches : branches;
			missedBranches = other.missedBranches < missedBranches ? other.missedBranches : missedBranches;
		...
		BNCH_SWT_ALWAYS_INLINE performance_counters& operator/=(double numerator) {
			cycles /= numerator;
			branches /= numerator;
			missedBranches /= numerator;
			instructions /= numerator;
			return *this;
		}
	};

	BNCH_SWT_ALWAYS_INLINE performance_counters operator-(const performance_counters& a, const performance_counters& b) {
		return performance_counters(a.cycles - b.cycles, a.branches - b.branches, a.missedBranches - b.missedBranches, a.instructions - b.instructions);
	}
	...
	static int32_t (*kpc_get_cpu_counters)(bool all_cpus, u32 classes, int32_t* curcpu, u64* buf);
	...
	static int32_t kperf_lightweight_pet_get(u32* enabled) {
		if (!enabled) {
			return -1;
		}
		usize size = 4;
	...
	static int32_t kperf_lightweight_pet_set(u32 enabled) {
	...
	/// @return kpep_config_error_code, 0 for success.
	static int32_t (*kpep_config_add_event)(kpep_config* cfg, kpep_event** ev_ptr, u32 flag, u32* err);
	...
	/// @return kpep_config_error_code, 0 for success.
	static int32_t (*kpep_config_events)(kpep_config* cfg, kpep_event** buf, usize buf_size);
	...
	/// @return kpep_config_error_code, 0 for success.
	static int32_t (*kpep_config_kpc)(kpep_config* cfg, kpc_config_t* buf, usize buf_size);
 ...
 */
	...
	/// @return 0 on success.
	static int32_t kdebug_reset(void) {
	...
	/// @return 0 on success.
	static int32_t kdebug_reinit(void) {
		int32_t mib[3] = { CTL_KERN, KERN_KDEBUG, KERN_KDSETUP };
		return sysctl(mib, 3, NULL, NULL, NULL, 0);
	}

	/// Set debug filter.
	static int32_t kdebug_setreg(kd_regtype* kdr) {
	...
	/// @return 0 on success.
	static int32_t kdebug_trace_setbuf(int32_t nbufs) {
	...
	/// @return 0 on success.
	static int32_t kdebug_trace_enable(bool enable) {
	...
	/// @return 0 on success.
	static int32_t kdebug_get_bufinfo(kbufinfo_t* info) {
		if (!info) {
			return -1;
		}
	...
	/// @return 0 on success.
	static int32_t kdebug_trace_read(void* buf, usize len, usize* count) {
		if (count) {
			*count = 0;
		}
		if (!buf || !len) {
	...
	/// @return 0 on success.
	static int32_t kdebug_wait(usize timeout_ms, bool* suc) {
		if (timeout_ms == 0) {
			return -1;
		}
	...
	BNCH_SWT_ALWAYS_INLINE performance_counters get_counters() {
	...
	template<typename event_count> struct event_collector_type {
		std::chrono::time_point<clock_type> startClock{};
		performance_counters diff{};
		event_count count{};

		BNCH_SWT_ALWAYS_INLINE event_collector_type() : diff(0) {
		...
		template<typename function_type, typename... arg_types> BNCH_SWT_ALWAYS_INLINE event_count start(function_type&& function, arg_types&&... args) {
			...
			if (has_events()) {
				...
				diff					 = end - diff;
				count.cyclesVal.emplace(diff.cycles);
				count.instructionsVal.emplace(diff.instructions);
				count.branchesVal.emplace(diff.branches);
				count.missedBranchesVal.emplace(diff.missedBranches);
			}
