...
namespace bnch_swt {
	...
	BNCH_SWT_ALWAYS_INLINE static int32_t kperf_lightweight_pet_get(uint32_t* enabled) {
		if (!enabled) {
			return -1;
		}
		size_t size = 4;
	...
	BNCH_SWT_ALWAYS_INLINE static int32_t kperf_lightweight_pet_set(uint32_t enabled) {
 ...
 */
	...
	/// @return 0 on success.
	BNCH_SWT_ALWAYS_INLINE static int32_t kdebug_reset() {
	...
	/// @return 0 on success.
	BNCH_SWT_ALWAYS_INLINE static int32_t kdebug_reinit() {
		int32_t mib[3] = { CTL_KERN, KERN_KDEBUG, KERN_KDSETUP };
		return sysctl(mib, 3, NULL, NULL, NULL, 0);
	}

	/// Set debug filter.
	BNCH_SWT_ALWAYS_INLINE static int32_t kdebug_setreg(kd_regtype* kdr) {
	...
	/// @return 0 on success.
	BNCH_SWT_ALWAYS_INLINE static int32_t kdebug_trace_setbuf(int32_t nbufs) {
	...
	/// @return 0 on success.
	BNCH_SWT_ALWAYS_INLINE static int32_t kdebug_trace_enable(bool enable) {
	...
	/// @return 0 on success.
	BNCH_SWT_ALWAYS_INLINE static int32_t kdebug_get_bufinfo(kbufinfo_t* info) {
		if (!info) {
			return -1;
		}
	...
	/// @return 0 on success.
	BNCH_SWT_ALWAYS_INLINE static int32_t kdebug_trace_read(void* buf, size_t len, size_t* count) {
		if (count) {
			*count = 0;
		}
		if (!buf || !len) {
	...
	/// @return 0 on success.
	BNCH_SWT_ALWAYS_INLINE static int32_t kdebug_wait(size_t timeout_ms, bool* suc) {
		if (timeout_ms == 0) {
			return -1;
		}
	...
	BNCH_SWT_ALWAYS_INLINE performance_counters get_counters() {
     ...
     for (size_t i = 0; i < ev_count; i++) {
         ...
         uint64_t val = counters_1[idx] - counters_0[idx];
         printf("%14s: %llu\n", alias->alias, val);
     }*/
	...
	template<typename event_count> struct event_collector_type {
		...
		BNCH_SWT_ALWAYS_INLINE bool hasEvents() {
