...
namespace bnch_swt {
	...
	BNCH_SWT_INLINE size_t getL1CacheSize() {
		DWORD buffer_size							 = 0;
		...
		if (!buffer) {
			std::cerr << "Failed to allocate memory for processor information!" << std::endl;
			return 0;
		}
	...
	BNCH_SWT_INLINE void flushCache(void* ptr, size_t size) {
...
#elif defined(JSONIFIER_LINUX) || defined(JSONIFIER_MAC)
	BNCH_SWT_INLINE void flushCache(void* ptr, size_t size) {
		char* buffer = static_cast<char*>(ptr);
	...
	BNCH_SWT_INLINE void evictL1Cache() {
	...
	template<jsonifier_internal::string_literal stageNameNew, uint64_t maxExecutionCount = 100> struct benchmark_stage {
		...
		inline static std::vector<event_count> measuredEventsNew{ [] {
			...
			returnValues.resize(maxExecutionCount / 5);
			return returnValues;
		}() };
		...
		BNCH_SWT_INLINE static void warmupThread() {
		...
		BNCH_SWT_INLINE static const performance_metrics& runBenchmark(function_type&& functionNew, arg_types&&... args) {
			...
			while ((variation > threshold || i < maxExecutionCount)) {
				evictL1Cache();
				eventsNew[i]   = collector.start(std::forward<function_type>(functionNew), std::forward<arg_types>(args)...);
				if (i % (maxExecutionCount / 5) == 0 && i > 0) {
					resultsTemp = collectMetrics<subjectName>(i, maxExecutionCount / 5);
		...
		static performance_metrics collectMetrics(uint64_t offset, uint64_t measuredIterationCount) {
			static constexpr jsonifier_internal::string_literal benchmarkName{ benchmarkNameNew };
			...
			double volumeMB		   = eventsNew[offset].bytesProcessed.value() / (1024. * 1024.);
			double averageNs{ 0 };
			...
			double missedBranchesMin{ 0 };
			double missedBranchesAvg{ 0 };
			double cycles{};
			double instructions{};
			double branches{};
			double missedBranches{};
			std::copy(eventsNew.begin() + offset - measuredIterationCount, eventsNew.begin() + offset, measuredEventsNew.begin());
			for (const event_count& e: measuredEventsNew) {
				...
				missedBranchesAvg += missedBranches;
				missedBranchesMin = missedBranchesMin < missedBranches ? missedBranchesMin : missedBranches;
			}
			std::cout << "CURRENT NS: " << averageNs << std::endl;
			std::cout << "MIN NS: " << minNs << std::endl;
			metrics.timeInNs = averageNs;
			cyclesAvg /= measuredIterationCount;
			instructionsAvg /= measuredIterationCount;
			averageNs /= measuredIterationCount;
			branchesAvg /= measuredIterationCount;
			metrics.throughputMbPerSec.emplace(volumeMB * 1000000000 / averageNs);
			metrics.throughputVariation.emplace((averageNs - minNs) * 100.0 / averageNs);
			if (instructionsAvg != 0.0f) {
				metrics.instructionsPerByte.emplace(instructionsAvg / eventsNew[offset].bytesProcessed.value());
				metrics.instructionsPerCycle.emplace(instructionsAvg / cyclesAvg);
				metrics.instructionsPerExecution.emplace(instructionsAvg / measuredIterationCount);
				metrics.instructionsVariation.emplace((instructionsAvg - instructionsMin) * 100.0 / instructionsAvg);
			}
			if (cyclesAvg != 0.0f) {
				metrics.cyclesPerByte.emplace(cyclesAvg / eventsNew[offset].bytesProcessed.value());
				metrics.cyclesPerExecution.emplace(cyclesAvg / measuredIterationCount);
				metrics.cyclesVariation.emplace((cyclesAvg - cyclesMin) * 100.0 / cyclesAvg);
				metrics.frequencyGHz.emplace(cyclesMin / minNs);
			}
			if (branchesAvg != 0.0f) {
				metrics.branchMissesPerExecution.emplace(missedBranchesAvg / measuredIterationCount);
				metrics.branchesPerExecution.emplace(branchesAvg / measuredIterationCount);
			}
			return metrics;
		}
	};
