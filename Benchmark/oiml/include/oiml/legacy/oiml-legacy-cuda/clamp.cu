...
template<class T> static __global__ void op_clamp_kernel(const T* x, T* dst, const T min, const T max, const int k) {
...
template<class T> static void clamp_cuda(const T* x, T* dst, const T min, const T max, const int k, cudaStream_t stream) {
...
void oiml_cuda_op_clamp(oiml_backend_cuda_context& ctx, oiml_tensor* dst) {
	...
	if (src0->type == oiml::oiml_representation_types::float_16) {
		clamp_cuda(( const half* )src0_d, ( half* )dst_d, ( half )min, ( half )max, oiml_nelements(src0), stream);
	} else {
		clamp_cuda(( const float* )src0_d, ( float* )dst_d, ( float )min, ( float )max, oiml_nelements(src0), stream);
	}
}
