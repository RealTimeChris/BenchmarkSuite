...
static __global__ void cross_entropy_loss_f32(const float* __restrict__ logits, const float* __restrict__ labels, float* __restrict__ dst, const int nclasses, const int k) {
...
template<bool use_shared> static __global__ void cross_entropy_loss_back_f32(const float* __restrict__ grad, const float* __restrict__ logits, const float* __restrict__ labels,
...
void oiml_cuda_cross_entropy_loss(oiml_backend_cuda_context& ctx, oiml_tensor* dst) {
	...
	const size_t smpbo = oiml_cuda_info().devices[id].smpbo;

	oiml_cuda_pool_alloc<float> dst_tmp(pool, blocks_num.x);

	if (nbytes_shared <= smpbo) {
#if !(defined(OIML_USE_HIP) && defined(__HIP_PLATFORM_AMD__)) && !defined(OIML_USE_MUSA)
		static bool shared_memory_limit_raised[OIML_CUDA_MAX_DEVICES] = { false };
...
#endif// !(defined(OIML_USE_HIP) && defined(__HIP_PLATFORM_AMD__)) && !defined(OIML_USE_MUSA)
...
void oiml_cuda_cross_entropy_loss_back(oiml_backend_cuda_context& ctx, oiml_tensor* dst) {
	...
	const oiml_tensor* src0f = dst->src[1];
	const oiml_tensor* src1f = dst->src[2];

	OIML_ASSERT(src0f->type == oiml::oiml_representation_types::float_32);
	OIML_ASSERT(src1f->type == oiml::oiml_representation_types::float_32);
	OIML_ASSERT(grad->type == oiml::oiml_representation_types::float_32);
	OIML_ASSERT(dst->type == oiml::oiml_representation_types::float_32);

	OIML_ASSERT(oiml_is_scalar(grad));
	OIML_ASSERT(oiml_is_contiguous(src0f));
	OIML_ASSERT(oiml_is_contiguous(src1f));
	OIML_ASSERT(oiml_is_contiguous(dst));
	OIML_ASSERT(oiml_are_same_shape(src0f, src1f));
	OIML_ASSERT(oiml_are_same_shape(src0f, dst));
