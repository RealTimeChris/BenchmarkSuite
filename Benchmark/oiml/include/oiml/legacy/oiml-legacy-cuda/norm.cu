...
static __global__ void norm_f32(const float* x, float* dst, const int ncols, const int64_t stride_row, const int64_t stride_channel, const int64_t stride_sample, const float eps) {
...
template<int block_size> static __global__ void group_norm_f32(const float* x, float* dst, const int group_size, const int ne_elements, const float eps) {
...
template<int block_size> static __global__ void rms_norm_f32(const float* x, float* dst, const int ncols, const int64_t stride_row, const int64_t stride_channel,
...
template<int block_size> static __global__ void rms_norm_back_f32(const float* grad, const float* xf, float* dst, const int ncols, const float eps) {
...
static void norm_f32_cuda(const float* x, float* dst, const int ncols, const int nrows, const int nchannels, const int nsamples, const int64_t stride_row,
	const int64_t stride_channel, const int64_t stride_sample, const float eps, cudaStream_t stream) {
...
static void rms_norm_f32_cuda(const float* x, float* dst, const int ncols, const int nrows, const int nchannels, const int nsamples, const int64_t stride_row,
	const int64_t stride_channel, const int64_t stride_sample, const float eps, cudaStream_t stream) {
	const dim3 blocks_num(nrows, nchannels, nsamples);
	if (ncols < 1024) {
		const dim3 block_dims(WARP_SIZE, 1, 1);
...
void oiml_cuda_op_norm(oiml_backend_cuda_context& ctx, oiml_tensor* dst) {
	...
	const size_t ts0 = oiml_type_size(src0->type);
	OIML_ASSERT(nb00 == ts0);
...
void oiml_cuda_op_group_norm(oiml_backend_cuda_context& ctx, oiml_tensor* dst) {
	...
	group_norm_f32_cuda(src0_d, dst_d, num_groups * src0->ne[3], eps, group_size, oiml_nelements(src0), stream);
}
...
void oiml_cuda_op_rms_norm_back(oiml_backend_cuda_context& ctx, oiml_tensor* dst) {
	const oiml_tensor* grad	 = dst->src[0];// gradients
	const oiml_tensor* src0f = dst->src[1];// src0 from forward pass
	...
	OIML_ASSERT(grad->type == oiml::oiml_representation_types::float_32);
	OIML_ASSERT(src0f->type == oiml::oiml_representation_types::float_32);
	OIML_ASSERT(dst->type == oiml::oiml_representation_types::float_32);

	const int64_t ne00	= src0f->ne[0];
	const int64_t nrows = oiml_nrows(src0f);
