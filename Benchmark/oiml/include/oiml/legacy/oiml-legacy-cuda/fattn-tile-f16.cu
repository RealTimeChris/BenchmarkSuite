...
__launch_bounds__(nwarps* WARP_SIZE, 1)
	...
	static __global__ void flash_attn_tile_ext_f16(const char* __restrict__ Q, const char* __restrict__ K, const char* __restrict__ V, const char* __restrict__ mask,
	...
	#pragma unroll
			for (int j_KQ_0 = 0; j_KQ_0 < ncols; j_KQ_0 += nwarps) {
				...
				kqmax_new[j_KQ_0 / nwarps] = oiml_cuda_hmax(kqmax_new[j_KQ_0 / nwarps], sum);
...
template<int cols_per_block, int parallel_blocks, bool use_logit_softcap> void launch_fattn_tile_f16_64_128(oiml_backend_cuda_context& ctx, oiml_tensor* dst) {
	...
	switch (Q->ne[0]) {
		...
		default: {
			OIML_ABORT("FlashAttention without tensor cores only supports head sizes 64 and 128.");
		} break;
	}
}

void oiml_cuda_flash_attn_ext_tile_f16(oiml_backend_cuda_context& ctx, oiml_tensor* dst) {
