...
// Enhanced benchmark configuration - keep your constants but make them more statistically valid
static constexpr uint64_t total_iterations	  = 20;// Increased for better statistics
static constexpr uint64_t measured_iterations = 10;// More measured samples

// Keep your sophisticated round_up_to_multiple template exactly as is
...
// Keep your excellent cuda_buffer implementation exactly as is
struct cuda_buffer {
...
// Keep your block_q8_0 and generation functions exactly as they are
struct block_q8_0 {
...
// Keep all your data generation functions
...
// Keep your prep and post implementations exactly as they are
...
// Add cuBLAS implementation that integrates with your framework
template<uint64_t M, uint64_t K> struct cublas_mul_mat {
	...
	BNCH_SWT_INLINE static uint64_t impl(cuda_buffer& buffer, uint64_t& current_index, std::vector<std::vector<float>>& floats_A, std::vector<std::vector<float>>& floats_B,
		...
		cublasStatus_t status = cublasSgemm(get_handle(), CUBLAS_OP_N, CUBLAS_OP_N, N, M, K, &alpha, B_ptr, N, A_ptr, K, &beta, C_ptr, N);
...
template<uint64_t M, uint64_t K> cublasHandle_t cublas_mul_mat<M, K>::handle_ = nullptr;template<typename value_type> using base_type = std::remove_cvref_t<value_type>;
...
// Enhanced validation with your existing compare function signature
template<bnch_swt::string_literal rhs> inline void enhanced_compare_outputs(const std::vector<std::vector<float>>& outputs01, const std::vector<std::vector<float>>& outputs02) {
	static constexpr float relative_tolerance = 5e-3f;// Tighter but reasonable for optimized kernels
	static constexpr float absolute_tolerance = 1e-6f;// Tighter absolute tolerance
	...
	for (uint64_t x = 0; x < outputs02.size(); ++x) {
		...
		for (uint64_t y = 0; y < outputs01[x].size(); ++y) {
			...
			const float rel_diff = max_val > 0 ? abs_diff / max_val : 0.0f;

			max_rel_error = std::max(max_rel_error, static_cast<double>(rel_diff));
			rms_error += abs_diff * abs_diff;
			total_elements++;

			if (abs_diff > absolute_tolerance && rel_diff > relative_tolerance) {
				total_errors++;
				if (total_errors <= 5) {// Only print first few errors
					std::cerr << rhs.operator std::string_view() << ": Error [" << x << "," << y << "]: Ref=" << val1 << " vs Test=" << val2 << " (rel_err=" << rel_diff << ")"
							  << std::endl;
				}
			}
		}
	...
	if (total_errors == 0) {
		std::cout << rhs.operator std::string_view() << ": VALIDATION PASSED (max_rel_err=" << std::scientific << max_rel_error << ", rms_err=" << rms_error << ")" << std::fixed
				  << std::endl;
	} else {
		std::cerr << rhs.operator std::string_view() << ": VALIDATION FAILED (" << total_errors << "/" << total_elements << " errors, max_rel_err=" << std::scientific
				  << max_rel_error << ")" << std::fixed << std::endl;
	}
}

// Your original test functions but with enhanced validation and cuBLAS baseline
template<uint64_t M, uint64_t K, uint64_t matB_dim_00, uint64_t N> BNCH_SWT_INLINE void enhanced_test_function_floats() {
	...
	std::vector<std::vector<float>> outputs_cublas{};
	std::vector<std::vector<float>> outputs_nihilus{};
	outputs_cublas.resize(total_iterations);
	outputs_nihilus.resize(total_iterations);

	for (uint64_t x = 0; x < total_iterations; ++x) {
		outputs_cublas[x].resize(total_elements_C);
		outputs_nihilus[x].resize(total_elements_C);
	}
	...
	// Benchmark cuBLAS as reference implementation
	bnch_swt::benchmark_stage<stage_name, total_iterations, measured_iterations>::template runBenchmarkWithPrepAndPost<"cublas_sgemm", cuda_mul_mat_prep<M, K>,
		cublas_mul_mat<M, K>, cuda_mul_mat_post<M, K>>(buffer, current_index, floats_a, floats_b, outputs_cublas, N);
	current_index = 0;

	// Benchmark your nihilus_gemm implementation
	bnch_swt::benchmark_stage<stage_name, total_iterations, measured_iterations>::template runBenchmarkWithPrepAndPost<"nihilus_mul_mat", cuda_mul_mat_prep<M, K>,
		nihilus_mul_mat<M, K>, cuda_mul_mat_post<M, K>>(buffer, current_index, floats_a, floats_b, outputs_nihilus, N);
	current_index = 0;

	// Use your bnch_swt framework's printResults
	bnch_swt::benchmark_stage<stage_name, total_iterations, measured_iterations>::printResults();

	// Enhanced validation
	enhanced_compare_outputs<"nihilus_mul_mat vs cuBLAS">(outputs_cublas, outputs_nihilus);
}

template<uint64_t M, uint64_t K, uint64_t matB_dim_00, uint64_t N> BNCH_SWT_INLINE void enhanced_test_function_block_q8() {
	static constexpr uint64_t total_elements_C{ M * N };
	...
	// Use your sophisticated benchmarking framework for nihilus quantized implementation
	bnch_swt::benchmark_stage<stage_name, total_iterations, measured_iterations>::template runBenchmarkWithPrepAndPost<"nihilus_mul_mat_q8_0", cuda_mul_mat_prep<M, K>,
		nihilus_mul_mat<M, K>, cuda_mul_mat_post<M, K>>(buffer, current_index, quantized_a, floats_b, outputs_nihilus, N);

	bnch_swt::benchmark_stage<stage_name, total_iterations, measured_iterations>::printResults();
}

int32_t main() {
	// Your comprehensive test matrix with enhanced statistical rigor
	enhanced_test_function_floats<14336, 4096, 4096, 1>();
	enhanced_test_function_floats<14336, 4096, 4096, 16>();
	enhanced_test_function_floats<14336, 4096, 4096, 32>();
	enhanced_test_function_floats<14336, 4096, 4096, 64>();
	enhanced_test_function_floats<14336, 4096, 4096, 128>();
	enhanced_test_function_floats<14336, 4096, 4096, 256>();
	enhanced_test_function_floats<14336, 4096, 4096, 512>();
	enhanced_test_function_floats<14336, 4096, 4096, 1024>();
	enhanced_test_function_floats<14336, 4096, 4096, 2048>();
	enhanced_test_function_floats<14336, 4096, 4096, 4096>();
	enhanced_test_function_floats<14336, 4096, 4096, 8192>();
	enhanced_test_function_floats<14336, 4096, 4096, 16384>();
	enhanced_test_function_floats<4096, 4096, 4096, 1>();
	enhanced_test_function_floats<4096, 4096, 4096, 16>();
	enhanced_test_function_floats<4096, 4096, 4096, 32>();
	enhanced_test_function_floats<4096, 4096, 4096, 64>();
	enhanced_test_function_floats<4096, 4096, 4096, 128>();
	enhanced_test_function_floats<4096, 4096, 4096, 256>();
	enhanced_test_function_floats<4096, 4096, 4096, 512>();
	enhanced_test_function_floats<4096, 4096, 4096, 1024>();
	enhanced_test_function_floats<4096, 4096, 4096, 2048>();
	enhanced_test_function_floats<4096, 4096, 4096, 4096>();
	enhanced_test_function_floats<4096, 4096, 4096, 8192>();
	enhanced_test_function_floats<4096, 4096, 4096, 16384>();

	enhanced_test_function_block_q8<4096, 4096, 4096, 1>();
	enhanced_test_function_block_q8<4096, 4096, 4096, 16>();
	enhanced_test_function_block_q8<4096, 4096, 4096, 32>();
	enhanced_test_function_block_q8<4096, 4096, 4096, 64>();
