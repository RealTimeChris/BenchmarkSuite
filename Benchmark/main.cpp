...
inline static constexpr std::array<size_t, 12> sizes{ 1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048 };

template<typename value_type>
BNCH_SWT_INLINE bool operator!=(const std::vector<value_type, oiml::alloc_wrapper<value_type>>& lhs, const std::vector<value_type, oiml::alloc_wrapper<value_type>>& rhs) {
...
template<size_t dim_a, size_t dim_b, size_t dim_c> struct test {
	...
	struct simple_model_ggml {
		...
		using matrix_type_b = oiml::oiml_get_tensor_type_t<device_type, oiml::oiml_representation_types::q8_0, oiml::oiml_op_type::oiml_op_mul_mat, matrix_b_dim_a, matrix_b_dim_b>;
		...
		simple_model_ggml(const void* data_new_01, const void* data_new_02, float* data_new_03) : matrix_a{}, matrix_b{}, result_matrix{ data_new_03 } {
			simple_model_ggml::device_type::execute_unary_op<oiml::oiml_op_type::oiml_op_load>(matrix_a, ( typename matrix_type_a::value_type*& )(data_new_01));
			simple_model_ggml::device_type::execute_unary_op<oiml::oiml_op_type::oiml_op_load>(matrix_b, ( typename matrix_type_b::value_type*& )(data_new_02));
		};
		matrix_type_result result_matrix{};
	};

	struct simple_model_ggml_dynamic {
		using device_type = oiml::oiml_backend_device<oiml_backend_device_types::cpu>;
		oiml::oiml_dynamic_tensor matrix_a{};
		oiml::oiml_dynamic_tensor matrix_b{};

		simple_model_ggml_dynamic(const void* data_new_01, const void* data_new_02)
			: matrix_a{ oiml::oiml_representation_types::q8_0, matrix_a_dim_a, matrix_a_dim_b }, matrix_b{ oiml::oiml_representation_types::q8_0, matrix_b_dim_a, matrix_b_dim_b } {
			simple_model_ggml::device_type::execute_unary_op<oiml::oiml_op_type::oiml_op_load>(matrix_a, ( oiml::block_q8_0<oiml_half>*& )(data_new_01));
			simple_model_ggml::device_type::execute_unary_op<oiml::oiml_op_type::oiml_op_load>(matrix_b, ( float*& )(data_new_02));
		};
	...
	BNCH_SWT_INLINE static void compute_graph_ggml(simple_model_ggml& model) {
		simple_model_ggml::device_type::template execute_binary_op<oiml::oiml_op_type::oiml_op_mul_mat>(model.matrix_a, model.matrix_b, model.result_matrix);
	}

	BNCH_SWT_INLINE static void compute_graph_ggml_dynamic(simple_model_ggml_dynamic& model, float* out_data) {
		simple_model_ggml_dynamic::device_type::template execute_binary_op<oiml::oiml_op_type::oiml_op_mul_mat>(model.matrix_a, model.matrix_b, model.result_matrix);
		simple_model_ggml_dynamic::device_type::execute_unary_op<oiml::oiml_op_type::oiml_op_store>(model.result_matrix, out_data);
	}

	BNCH_SWT_INLINE static void compute_graph_prep(simple_model& model, void* matrix_a, void* matrix_b) {
#if defined(GGML_USE_CUDA)
		model.backend = ggml_backend_cuda_init(0);
#else
		model.backend = ggml_backend_init_best();
#endif
		model.params   = { ggml_tensor_overhead() * num_tensors, nullptr, true };
		model.ctx	   = ggml_init(model.params);
		model.matrix_a = ggml_new_tensor_2d(model.ctx, GGML_TYPE_Q8_0, matrix_a_dim_a, matrix_a_dim_b);
		model.matrix_b = ggml_new_tensor_2d(model.ctx, GGML_TYPE_Q8_0, matrix_b_dim_a, matrix_b_dim_b);
		model.buffer   = ggml_backend_alloc_ctx_tensors(model.ctx, model.backend);
		ggml_backend_tensor_set(model.matrix_a, matrix_a, 0, ggml_nbytes(model.matrix_a));
		ggml_backend_tensor_set(model.matrix_b, matrix_b, 0, ggml_nbytes(model.matrix_b));
		model.allocr   = ggml_gallocr_new(ggml_backend_get_default_buffer_type(model.backend));
		model.buf_size = ggml_tensor_overhead() * GGML_DEFAULT_GRAPH_SIZE + ggml_graph_overhead();
		model.buf.resize(model.buf_size);
		model.params   = { model.buf_size, model.buf.data(), true };
		model.ctx0	   = ggml_init(model.params);
		model.gf	   = ggml_new_graph(model.ctx0);
		model.result02 = ggml_mul_mat(model.ctx0, model.matrix_a, model.matrix_b);
		ggml_build_forward_expand(model.gf, model.result02);
		ggml_free(model.ctx0);
		ggml_gallocr_reserve(model.allocr, model.gf);
		ggml_gallocr_alloc_graph(model.allocr, model.gf);
		if (ggml_backend_is_cpu(model.backend)) {
			ggml_backend_cpu_set_n_threads(model.backend, 6);
		}
	}

	BNCH_SWT_INLINE static void compute_graph(simple_model& model, float* out_data) {
		ggml_backend_graph_compute(model.backend, model.gf);
		ggml_backend_tensor_get(model.result02, out_data, 0, ggml_nbytes(model.result02));
	}

	// Function to clean up resources
	BNCH_SWT_INLINE static void compute_graph_cleanup(simple_model& model) {
	...
	BNCH_SWT_INLINE static void compute_graph_prep_oiml(simple_model_oiml& model, void* matrix_a, void* matrix_b) {
#if defined(OIML_USE_CUDA)
		model.backend = oiml_backend_cuda_init(0);
#else
		model.backend = oiml_backend_init_best();
#endif
		...
		model.matrix_b = oiml_new_tensor_2d(model.ctx, oiml::oiml_representation_types::q8_0, matrix_b_dim_a, matrix_b_dim_b);
	...
	BNCH_SWT_INLINE static void compute_graph_oiml(simple_model_oiml& model, float* out_data) {
	...
	BNCH_SWT_INLINE static void compute_graph_cleanup_oiml(simple_model_oiml& model) {
	...
	struct test_data {
		...
		BNCH_SWT_INLINE test_data() {
			matrix_a.resize(simple_model_ggml::matrix_type_a::get_total_bytes());
			matrix_b.resize(simple_model_ggml::matrix_type_b::get_total_bytes());
			out_data01.resize(simple_model_ggml::matrix_type_result::get_total_bytes() / 4);
			out_data02.resize(simple_model_ggml::matrix_type_result::get_total_bytes() / 4);
			out_data03.resize(simple_model_ggml::matrix_type_result::get_total_bytes() / 4);
			for (size_t x = 0; x < simple_model_ggml::matrix_type_a::get_total_bytes(); ++x) {
				matrix_a[x] = bnch_swt::random_generator::generateValue<uint8_t>() / 4;
			}

			for (size_t x = 0; x < simple_model_ggml::matrix_type_b::get_total_bytes(); ++x) {
	...
	BNCH_SWT_INLINE static void run() {
		...
		{
			/*
			std::vector<simple_model_ggml> testModelsOiml{};
			for (size_t x = 0; x < max_iteration_count; ++x) {
			...
			bnch_swt::benchmark_stage<name, max_iteration_count, measured_iteration_count>::template runBenchmark<"oiml-threadpool", "cyan">([&] {
				compute_graph_ggml(testModelsOiml[currentIndex]);
				auto new_size = testData[currentIndex].matrix_a.size() * 4;
		...
		{
			currentIndex = 0;
			std::vector<simple_model_oiml> testModels{ max_iteration_count };
			for (size_t x = 0; x < max_iteration_count; ++x) {
				compute_graph_prep_oiml(testModels[x], testData[x].matrix_a.data(), testData[x].matrix_b.data());
			}
			bnch_swt::benchmark_stage<name, max_iteration_count, measured_iteration_count>::template runBenchmark<"oiml-threadpool", "cyan">([&] {
				compute_graph_oiml(testModels[currentIndex], testData[currentIndex].out_data02.data());
				auto new_size = testData[currentIndex].matrix_a.size() * 4;
				bnch_swt::doNotOptimizeAway(testData[currentIndex].out_data02.data());
				++currentIndex;
				return new_size;
			...
			for (size_t x = 0; x < max_iteration_count; ++x) {
				compute_graph_cleanup_oiml(testModels[x]);
			}
		}
		for (size_t x = 0; x < max_iteration_count; ++x) {
			if (std::this_thread::get_id() == std::thread::id{}) {
				print_values<matrix_c_dim_a, matrix_c_dim_b>(testData[x].out_data02.data(), "ggml-tensor");
				print_values<matrix_c_dim_a, matrix_c_dim_b>(testData[x].out_data01.data(), "ggml-tensor");
			}
			if (operator!=(testData[x].out_data01, testData[x].out_data02)) {
...
int32_t main() {
	//test<32, 32, 32>::run();
	test<sizes[5], sizes[5], sizes[5]>::run();
