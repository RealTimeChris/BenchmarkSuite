...
struct alignas(32) block_q8_0_new {
	alignas(32) int8_t qs[n / 8];
	int16_t d;
};

JSONIFIER_INLINE void ggml_vec_dot_q8_0_f32_unaligned_float(float* s, const void* vx, const void* vy) {
...
JSONIFIER_INLINE void ggml_vec_dot_q8_0_f32_unaligned_int8(float* s, const block_q8_0* vx, const void* vy) {
	static constexpr int qk = 32;
	static constexpr int nb = n / qk;
	assert(n % qk == 0);

	const block_q8_0* x = vx;
	const float* y		= ( float* )vy;
	...
	for (int32_t ib = 0; ib < nb - 4; ib += 4) {
		...
		__m256 temp0 = _mm256_cvtepi32_ps(_mm256_cvtepi16_epi32(_mm256_castsi256_si128(x0_0_0_0)));
		__m256 y0	 = _mm256_castsi256_ps(_mm256_loadu_si256(( __m256i* )y));
		local_sum	 = _mm256_fmadd_ps(temp0, y0, local_sum);

		temp0	  = _mm256_cvtepi32_ps(_mm256_cvtepi16_epi32(_mm256_extracti128_si256(x0_0_0_0, 1)));
		y0		  = _mm256_castsi256_ps(_mm256_loadu_si256(( __m256i* )(y + 8)));
		local_sum = _mm256_fmadd_ps(temp0, y0, local_sum);

		temp0	  = _mm256_cvtepi32_ps(_mm256_cvtepi16_epi32(_mm256_castsi256_si128(x0_1_0_0)));
		y0		  = _mm256_castsi256_ps(_mm256_loadu_si256(( __m256i* )(y + 16)));
		local_sum = _mm256_fmadd_ps(temp0, y0, local_sum);
		...
		temp0	  = _mm256_cvtepi32_ps(_mm256_cvtepi16_epi32(_mm256_castsi256_si128(x0_0_0_0)));
		y0		  = _mm256_castsi256_ps(_mm256_loadu_si256(( __m256i* )y));
		local_sum = _mm256_fmadd_ps(temp0, y0, local_sum);
		...
		temp0	  = _mm256_cvtepi32_ps(_mm256_cvtepi16_epi32(_mm256_extracti128_si256(x0_1_0_0, 1)));
		y0		  = _mm256_castsi256_ps(_mm256_loadu_si256(( __m256i* )(y + 24)));
		local_sum = _mm256_fmadd_ps(temp0, y0, local_sum);
...
JSONIFIER_INLINE void ggml_vec_dot_q8_0_f32_aligned_float(float* s, const block_q8_0_new* vx, const void* vy) {
	static constexpr int qk = 32;
	static constexpr int nb = n / qk;
	...
	const block_q8_0_new* x = ( const block_q8_0_new* )vx;
	const float* y		= ( const float* )vy;
...
JSONIFIER_INLINE void ggml_vec_dot_q8_0_f32_aligned_int8(float* s, const block_q8_0_new* vx, const int8_t* vy) {
	static constexpr int qk = 32;
	static constexpr int nb = n / qk;
	assert(n % qk == 0);

	const block_q8_0_new* x = vx;
	const float* y		= ( float* )vy;
	...
	for (int32_t ib = 0; ib < nb - 4; ib += 4) {
		...
		__m256 temp0 = _mm256_cvtepi32_ps(_mm256_cvtepi16_epi32(_mm256_castsi256_si128(x0_0_0_0)));
		__m256 y0	 = _mm256_castsi256_ps(_mm256_load_si256(( __m256i* )y));
		local_sum	 = _mm256_fmadd_ps(temp0, y0, local_sum);

		temp0	  = _mm256_cvtepi32_ps(_mm256_cvtepi16_epi32(_mm256_extracti128_si256(x0_0_0_0, 1)));
		y0		  = _mm256_castsi256_ps(_mm256_load_si256(( __m256i* )(y + 8)));
		local_sum = _mm256_fmadd_ps(temp0, y0, local_sum);

		temp0	  = _mm256_cvtepi32_ps(_mm256_cvtepi16_epi32(_mm256_castsi256_si128(x0_1_0_0)));
		y0		  = _mm256_castsi256_ps(_mm256_load_si256(( __m256i* )(y + 16)));
		local_sum = _mm256_fmadd_ps(temp0, y0, local_sum);
		...
		temp0	  = _mm256_cvtepi32_ps(_mm256_cvtepi16_epi32(_mm256_castsi256_si128(x0_0_0_0)));
		y0		  = _mm256_castsi256_ps(_mm256_load_si256(( __m256i* )y));
		local_sum = _mm256_fmadd_ps(temp0, y0, local_sum);
		...
		temp0	  = _mm256_cvtepi32_ps(_mm256_cvtepi16_epi32(_mm256_extracti128_si256(x0_1_0_0, 1)));
		y0		  = _mm256_castsi256_ps(_mm256_load_si256(( __m256i* )(y + 24)));
		local_sum = _mm256_fmadd_ps(temp0, y0, local_sum);
...
template<bnch_swt::string_literal testNameNew> BNCH_SWT_INLINE void testFunction() noexcept {
	...
	bnch_swt::benchmark_stage<testName, maxIterations, measuredIterations>::template runBenchmark<"unaligned-float", "cyan">([&] {
		...
		ggml_vec_dot_q8_0_f32_unaligned_float(&testDataResults01[currentIndex], testData[currentIndex].orig_x.get(), testData[currentIndex].orig_x.get()->qs);
		bytesProcessed += n;
	...
	bnch_swt::benchmark_stage<testName, maxIterations, measuredIterations>::template runBenchmark<"unaligned-int8", "cyan">([&] {
		uint64_t bytesProcessed{};
		ggml_vec_dot_q8_0_f32_unaligned_int8(&testDataResults01[currentIndex], testData[currentIndex].orig_x.get(),
			testData[currentIndex].orig_x.get()->qs);
		bytesProcessed += n;
		bnch_swt::doNotOptimizeAway(testDataResults01[currentIndex]);
		++currentIndex;
		return bytesProcessed;
	...
	bnch_swt::benchmark_stage<testName, maxIterations, measuredIterations>::template runBenchmark<"aligned-float", "cyan">([&] {
		uint64_t bytesProcessed{};
		ggml_vec_dot_q8_0_f32_aligned_float(&testDataResults01[currentIndex], testData[currentIndex].orig_y.get(),
			testData[currentIndex].orig_y.get()->qs);
	...
	bnch_swt::benchmark_stage<testName, maxIterations, measuredIterations>::template runBenchmark<"aligned-int8", "cyan">([&] {
		uint64_t bytesProcessed{};
		ggml_vec_dot_q8_0_f32_aligned_int8(&testDataResults01[currentIndex], testData[currentIndex].orig_y.get(), testData[currentIndex].orig_y.get()->qs);
		bnch_swt::doNotOptimizeAway(testDataResults02[currentIndex]);
		bytesProcessed += n;
		++currentIndex;
		return bytesProcessed;
