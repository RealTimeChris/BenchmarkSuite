...
struct block_q8_0_mega_floats {
	alignas(32) float ds[QK8_0_MEGA_D];
};
...
constexpr size_t num_blocks_mega	= dim / sizeof(block_q8_0_mega_quants);
...
struct mega {
	alignas(32) std::unique_ptr<block_q8_0_mega_quants[]> w{ std::make_unique<block_q8_0_mega_quants[]>(num_blocks_mega) };
	alignas(32) std::unique_ptr<block_q8_0_mega_quants[]> x{ std::make_unique<block_q8_0_mega_quants[]>(num_blocks_mega) };
	alignas(32) std::unique_ptr<block_q8_0_mega_floats[]> y{ std::make_unique<block_q8_0_mega_floats[]>(num_blocks_mega) };
	alignas(32) std::unique_ptr<block_q8_0_mega_floats[]> z{ std::make_unique<block_q8_0_mega_floats[]>(num_blocks_mega) };
	mega() {
		...
		for (int i = 0; i < num_blocks_mega; ++i) {
			for (size_t j = 0; j < 8; ++j) {
				y[i].ds[j] = bnch_swt::random_generator::generateValue<uint8_t>();
				z[i].ds[j] = bnch_swt::random_generator::generateValue<uint8_t>();
			}
		}
	}
};
...
JSONIFIER_INLINE void oi_vec_dot_q8_0_q8_0_mega(const int ne, float* dst, const block_q8_0_mega_quants* __restrict x, const block_q8_0_mega_quants* __restrict y,
	const block_q8_0_mega_floats* __restrict x_x, const block_q8_0_mega_floats* __restrict y_x) {
	...
	for (int ib = 0; ib < nb; ++ib) {
		...
		float dx   = x_x[ib].ds[0];
		float dy   = y_x[ib].ds[0];
...
template<bnch_swt::string_literal testNameNew> BNCH_SWT_INLINE void testFunction() {
	...
	float dst{};
	{
		...
		bnch_swt::benchmark_stage<testName, maxIterations, 20>::template runBenchmark<"mega", "CYAN">([&] {
			...
			oi_vec_dot_q8_0_q8_0_mega(dim, &dst, reinterpret_cast<const block_q8_0_mega_quants*>(testDataMega[currentIndex].w.get()),
				reinterpret_cast<const block_q8_0_mega_quants*>(testDataMega[currentIndex].x.get()), testDataMega[currentIndex].y.get(), testDataMega[currentIndex].z.get());
			bytesProcessed += dim;
	...
	{
		std::array<aligned, maxIterations> testDataAligned{};
		currentIndex = 0ull;
		bnch_swt::benchmark_stage<testName, maxIterations, 20>::template runBenchmark<"aligned", "CYAN">([&] {
			uint64_t bytesProcessed{};
			oi_vec_dot_q8_0_q8_0_aligned(dim, &dst, reinterpret_cast<const block_q8_0_aligned_quants*>(testDataAligned[currentIndex].w.get()),
				reinterpret_cast<const block_q8_0_aligned_quants*>(testDataAligned[currentIndex].x.get()), testDataAligned[currentIndex].y.get(),
				testDataAligned[currentIndex].z.get());
			bytesProcessed += dim;
			bnch_swt::doNotOptimizeAway(dst);
			++currentIndex;
			return bytesProcessed;
