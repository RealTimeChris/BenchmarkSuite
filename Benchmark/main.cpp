...
static constexpr auto maxIterations{ 20 };
...
template<size_t count, bnch_swt::string_literal name> JSONIFIER_INLINE void testFunction32() {
	...
	size_t currentIndex{};
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, maxIterations, 4>::template runBenchmark<"alternative-digit-count-32", "cyan">([&]() {
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, maxIterations, 4>::template runBenchmark<"fast-digit-count-32", "cyan">([&]() {
		uint64_t currentCount{};
		for (size_t x = 0; x < count; ++x) {
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, maxIterations, 4>::template runBenchmark<"digit-count-32", "cyan">([&]() {
		uint64_t currentCount{};
		for (size_t x = 0; x < count; ++x) {
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, maxIterations, 4>::template runBenchmark<"rtc-32", "cyan">([&]() {
		uint64_t currentCount{};
		for (size_t x = 0; x < count; ++x) {
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, maxIterations, 4>::printResults(true, true);
}
...
template<size_t count, bnch_swt::string_literal name> JSONIFIER_INLINE void testFunction64() {
	...
	size_t currentIndex{};
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, maxIterations, 4>::template runBenchmark<"alternative-digit-count-64", "cyan">([&]() {
		uint64_t currentCount{};
		for (size_t x = 0; x < count; ++x) {
			auto newCount			 = alternative_digit_count(randomIntegers[currentIndex][x]);
			results[currentIndex][x] = newCount;
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, maxIterations, 4>::template runBenchmark<"fast-digit-count-64", "cyan">([&]() {
		uint64_t currentCount{};
		for (size_t x = 0; x < count; ++x) {
			auto newCount			 = fast_digit_count(randomIntegers[currentIndex][x]);
			results[currentIndex][x] = newCount;
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, maxIterations, 4>::template runBenchmark<"digit-count-64", "cyan">([&]() {
		uint64_t currentCount{};
		for (size_t x = 0; x < count; ++x) {
			auto newCount			 = digit_count(randomIntegers[currentIndex][x]);
			results[currentIndex][x] = newCount;
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, maxIterations, 4>::template runBenchmark<"eisenwave-logfloor", "cyan">([&]() {
		uint64_t currentCount{};
		for (size_t x = 0; x < count; ++x) {
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, maxIterations, 4>::template runBenchmark<"rtc-64-countl-zero", "cyan">([&]() {
		uint64_t currentCount{};
		for (size_t x = 0; x < count; ++x) {
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, maxIterations, 4>::template runBenchmark<"rtc-64", "cyan">([&]() {
		uint64_t currentCount{};
		for (size_t x = 0; x < count; ++x) {
			auto newCount			 = fastDigitCount(randomIntegers[currentIndex][x]);
			results[currentIndex][x] = newCount;
...
int main() {
	testFunction32<10000, "uint32-test-10000">();
	testFunction64<10000, "uint64-test-10000">();
	return 0;
}