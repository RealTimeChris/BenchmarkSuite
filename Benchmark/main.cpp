...
template<typename typeName> struct int_tables {
	...
	static constexpr uint16_t charTable02[]{ 0x3030u, 0x3130u, 0x3230u, 0x3330u, 0x3430u, 0x3530u, 0x3630u, 0x3730u, 0x3830u, 0x3930u, 0x3031u, 0x3131u, 0x3231u, 0x3331u, 0x3431u,
		0x3531u, 0x3631u, 0x3731u, 0x3831u, 0x3931u, 0x3032u, 0x3132u, 0x3232u, 0x3332u, 0x3432u, 0x3532u, 0x3632u, 0x3732u, 0x3832u, 0x3932u, 0x3033u, 0x3133u, 0x3233u, 0x3333u,
		0x3433u, 0x3533u, 0x3633u, 0x3733u, 0x3833u, 0x3933u, 0x3034u, 0x3134u, 0x3234u, 0x3334u, 0x3434u, 0x3534u, 0x3634u, 0x3734u, 0x3834u, 0x3934u, 0x3035u, 0x3135u, 0x3235u,
		0x3335u, 0x3435u, 0x3535u, 0x3635u, 0x3735u, 0x3835u, 0x3935u, 0x3036u, 0x3136u, 0x3236u, 0x3336u, 0x3436u, 0x3536u, 0x3636u, 0x3736u, 0x3836u, 0x3936u, 0x3037u, 0x3137u,
		0x3237u, 0x3337u, 0x3437u, 0x3537u, 0x3637u, 0x3737u, 0x3837u, 0x3937u, 0x3038u, 0x3138u, 0x3238u, 0x3338u, 0x3438u, 0x3538u, 0x3638u, 0x3738u, 0x3838u, 0x3938u, 0x3039u,
		0x3139u, 0x3239u, 0x3339u, 0x3439u, 0x3539u, 0x3639u, 0x3739u, 0x3839u, 0x3939u };
...
JSONIFIER_INLINE uint64_t fastDigitCount(const uint64_t inputValue) {
	const uint64_t originalDigitCount{ int_tables<void>::digitCounts[jsonifier::simd::lzcnt(inputValue)] };
...
template<BITMANIP_UNSIGNED_TYPENAME(Uint)> [[nodiscard]] JSONIFIER_INLINE constexpr Uint makeMask(Uint length) noexcept {
 ...
 */
template<BITMANIP_UNSIGNED_TYPENAME(Uint)> [[nodiscard]] JSONIFIER_INLINE constexpr bool isPow2or0(Uint val) noexcept {
	return (val & (val - 1)) == 0;
}
 ...
 * @see is_pow2_or_zero
 */
template<BITMANIP_UNSIGNED_TYPENAME(Uint)> [[nodiscard]] JSONIFIER_INLINE constexpr bool isPow2(Uint val) noexcept {
...
namespace detail {
	...
	template<typename Uint> [[nodiscard]] JSONIFIER_INLINE inline Uint ceilPow2m1_builtin(Uint v) noexcept {
		Uint log = log2floor_builtin(v);
		return v | (Uint{ 1 } << log) - Uint{ 1 };
	}
#endif

	template<typename Uint> [[nodiscard]] JSONIFIER_INLINE constexpr Uint ceilPow2m1_shift(Uint v) noexcept {
 ...
 */
template<BITMANIP_UNSIGNED_TYPENAME(Uint)> [[nodiscard]] JSONIFIER_INLINE constexpr Uint ceilPow2m1(Uint v) noexcept {
 ...
 * @param v the value to round up
 */
template<BITMANIP_UNSIGNED_TYPENAME(Uint)> [[nodiscard]] JSONIFIER_INLINE constexpr Uint ceilPow2(Uint v) noexcept {
 ...
 */
template<BITMANIP_UNSIGNED_TYPENAME(Uint)> [[nodiscard]] JSONIFIER_INLINE constexpr Uint floorPow2(Uint v) noexcept {
	return ceilPow2m1(v >> 1) + 1;
}
 ...
 */
template<typename Uint> [[nodiscard]] JSONIFIER_INLINE constexpr Uint log2floor_naive(Uint val) noexcept {
	Uint result = 0;
	while (val >>= 1) {
		++result;
	}
 ...
 * unsigned shift = (v > 0xFFFF) << 4;
    ...
    r |= shift;
 */
template<typename Uint> [[nodiscard]] JSONIFIER_INLINE constexpr Uint log2floor_fast(Uint v) noexcept {
	constexpr std::size_t iterations = log2bits_v<Uint>;
	unsigned result					 = 0;
 ...
 */
template<BITMANIP_UNSIGNED_TYPENAME(Uint)> [[nodiscard]] JSONIFIER_INLINE constexpr Uint log2floor(Uint v) noexcept {
#ifdef BITMANIP_HAS_BUILTIN_LOG2FLOOR
	if (not builtin::isconsteval()) {
 ...
 * @return the floored binary logarithm
 */
template<BITMANIP_UNSIGNED_TYPENAME(Uint)> [[nodiscard]] JSONIFIER_INLINE constexpr Uint log2ceil(Uint val) noexcept {
 ...
 */
template<BITMANIP_UNSIGNED_TYPENAME(Uint)> [[nodiscard]] JSONIFIER_INLINE constexpr Uint bitCount(Uint val) noexcept {
 ...
 */
	template<BITMANIP_UNSIGNED_TYPENAME(Uint)> [[nodiscard]] JSONIFIER_INLINE constexpr Uint logFloor_naive(Uint val, unsigned base) noexcept {
		Uint result = 0;
		while (val /= base) {
			++result;
		}
 ...
 */
	template<unsigned BASE, BITMANIP_UNSIGNED_TYPENAME(Uint)> [[nodiscard]] JSONIFIER_INLINE constexpr Uint logFloor_simple(Uint val) noexcept {
 ...
 */
	template<typename T, std::size_t N> struct [[nodiscard]] Table {
 ...
 */
	template<typename Uint, std::size_t BASE> [[nodiscard]] JSONIFIER_INLINE constexpr Table<unsigned char, bits_v<Uint>> makeGuessTable() noexcept {
	...
	[[nodiscard]] JSONIFIER_INLINE constexpr int compareApproximationToGuessTable(std::uint64_t approxFactor, const Table<unsigned char, SIZE>& table) noexcept {
 ...
 */
	template<std::size_t SIZE> [[nodiscard]] JSONIFIER_INLINE constexpr std::uint64_t approximateGuessTable(const Table<unsigned char, SIZE>& table) noexcept {
	...
	template<typename Uint, std::size_t BASE> struct [[nodiscard]] LogFloorGuesser {
	...
	template<typename Uint, std::size_t BASE, typename TableUint = nextLargerUintType<Uint>> [[nodiscard]] JSONIFIER_INLINE constexpr auto makePowerTable() noexcept {
 ...
 */
template<std::size_t BASE, BITMANIP_UNSIGNED_TYPENAME(Uint)> [[nodiscard]] JSONIFIER_INLINE constexpr Uint powConst(const Uint exponent) noexcept {
...
[[nodiscard]] JSONIFIER_INLINE constexpr auto logFloor(const Uint val) noexcept -> std::enable_if_t<(std::is_unsigned_v<Uint> && BASE >= 2), Uint> {
	if constexpr (isPow2(BASE)) {
...
template<size_t count, bnch_swt::string_literal name> JSONIFIER_INLINE void testFunction32() {
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, 16, 4>::template runBenchmark<"alternative-digit-count-32", "cyan">([&]() {
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, 16, 4>::template runBenchmark<"fast-digit-count-32", "cyan">([&]() {
		uint64_t currentCount{};
		for (size_t x = 0; x < count; ++x) {
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, 16, 4>::template runBenchmark<"digit-count-32", "cyan">([&]() {
		uint64_t currentCount{};
		for (size_t x = 0; x < count; ++x) {
			auto newCount = alternative_digit_count(randomIntegers[x]);
			results[x]	  = newCount;
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, 16, 4>::template runBenchmark<"rtc-32-bit", "cyan">([&]() {
		uint64_t currentCount{};
		for (size_t x = 0; x < count; ++x) {
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, 16, 4>::printResults(true, true);
}

template<size_t count, bnch_swt::string_literal name> JSONIFIER_INLINE void testFunction64() {
	const auto randomIntegers = generateRandomIntegers<uint64_t>(count, sizeof(uint64_t) == 4 ? 10 : 20);
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, 16, 4>::template runBenchmark<"fast-digit-count-64", "cyan">([&]() {
		uint64_t currentCount{};
		for (size_t x = 0; x < count; ++x) {
			auto newCount = fast_digit_count(randomIntegers[x]);
			results[x]	  = newCount;
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, 16, 4>::template runBenchmark<"alternative-digit-count-64", "cyan">([&]() {
		uint64_t currentCount{};
		for (size_t x = 0; x < count; ++x) {
			auto newCount = alternative_digit_count(randomIntegers[x]);
			results[x]	  = newCount;
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, 16, 4>::template runBenchmark<"digit-count-64", "cyan">([&]() {
		uint64_t currentCount{};
		for (size_t x = 0; x < count; ++x) {
			auto newCount = alternative_digit_count(randomIntegers[x]);
			results[x]	  = newCount;
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, 16, 4>::template runBenchmark<"eisenwave-logfloor-64-bit", "cyan">([&]() {
		uint64_t currentCount{};
		for (size_t x = 0; x < count; ++x) {
			auto newCount = fastDigitCountLogFloor(randomIntegers[x]);
			results[x]	  = newCount;
			currentCount += static_cast<uint64_t>(newCount);
		}
		bnch_swt::doNotOptimizeAway(currentCount);
	...
	bnch_swt::benchmark_stage<"compare-decimal-counting-functions-" + name, 16, 4>::template runBenchmark<"rtc-64-bit", "cyan">([&]() {
		uint64_t currentCount{};
		for (size_t x = 0; x < count; ++x) {
			auto newCount = fastDigitCount(randomIntegers[x]);
			results[x]	  = newCount;
