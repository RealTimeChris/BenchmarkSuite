...
#include "../crc/internal/crc32c_inline.h"
...
namespace absl {
	...
	class crc32c_t final {
	  public:
		...
		template<typename Sink> friend void AbslStringify(Sink& sink, crc32c_t crc) {
	...
	namespace crc_internal {
		...
		crc32c_t ExtendCrc32cInternal(crc32c_t initial_crc, absl::string_view buf_to_add);
	}// namespace crc_internal
	...
	inline crc32c_t ExtendCrc32c(crc32c_t initial_crc, absl::string_view buf_to_add) {
		...
		if (buf_to_add.size() <= 64) {
			...
			if (crc_internal::ExtendCrc32cInline(&crc, buf_to_add.data(), buf_to_add.size())) {
				return crc32c_t{ crc };
			}
		}
