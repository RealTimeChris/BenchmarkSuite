...
#include "../base/internal/endian.h"
#include "../base/macros.h"
#include "../base/nullability.h"
#include "../base/optimization.h"
#include "../crc/internal/crc_cord_state.h"
#include "../functional/function_ref.h"
#include "../meta/type_traits.h"
#include "../strings/cord_analysis.h"
#include "../strings/cord_buffer.h"
#include "../strings/internal/cord_data_edge.h"
#include "../strings/internal/cord_internal.h"
#include "../strings/internal/cord_rep_btree.h"
#include "../strings/internal/cord_rep_btree_reader.h"
#include "../strings/internal/cord_rep_crc.h"
#include "../strings/internal/cord_rep_flat.h"
#include "../strings/internal/cordz_info.h"
#include "../strings/internal/cordz_update_scope.h"
#include "../strings/internal/cordz_update_tracker.h"
#include "../strings/internal/string_constant.h"
...
namespace absl {
	...
	class Cord;
	class CordTestPeer;
	template<typename Releaser> Cord MakeCordFromExternal(absl::string_view, Releaser&&);
	...
	class Cord {
	  private:
		template<typename T> using EnableIfString = absl::enable_if_t<std::is_same<T, std::string>::value, int>;
	  ...
	  public:
		...
		//   Bar(c);
		// }
		template<typename Releaser> friend Cord MakeCordFromExternal(absl::string_view data, Releaser&& releaser);
		...
		CordBuffer GetCustomAppendBuffer(size_t block_size, size_t capacity, size_t min_capacity = 16);
		...
		friend void CopyCordToString(const Cord& src, absl::Nonnull<std::string*> dst);
		...
		friend void AppendCordToString(const Cord& src, absl::Nonnull<std::string*> dst);
		...
		class CharIterator {
		  ...
		  private:
			explicit CharIterator(absl::Nonnull<const Cord*> cord) : chunk_iterator_(cord) {
			}
		...
		absl::optional<absl::string_view> TryFlat() const ABSL_ATTRIBUTE_LIFETIME_BOUND;
		...
		friend void AbslFormatFlush(absl::Nonnull<absl::Cord*> cord, absl::string_view part) {
			cord->Append(part);
		}

		// Support automatic stringification with absl::StrCat and absl::StrFormat.
		template<typename Sink> friend void AbslStringify(Sink& sink, const absl::Cord& cord) {
			for (absl::string_view chunk: cord.Chunks()) {
				sink.Append(chunk);
			}
		}
	  ...
	  private:
		...
		static inline std::strong_ordering ConvertCompareResultToStrongOrdering(int c) {
			if (c == 0) {
		...
		friend inline std::strong_ordering operator<=>(const Cord& lhs, absl::string_view rhs) {
			return ConvertCompareResultToStrongOrdering(lhs.Compare(rhs));
		}

		friend inline std::strong_ordering operator<=>(absl::string_view lhs, const Cord& rhs) {
			return ConvertCompareResultToStrongOrdering(-rhs.Compare(lhs));
		}
#endif

		friend absl::Nullable<const CordzInfo*> GetCordzInfoForTesting(const Cord& cord);
		...
		class InlineRep {
		  public:
			...
			explicit constexpr InlineRep(absl::string_view sv, absl::Nullable<CordRep*> rep);
			...
			void SetTreeOrEmpty(absl::Nullable<CordRep*> rep, const CordzUpdateScope& scope);
			...
			void EmplaceTree(absl::Nonnull<CordRep*> rep, const InlineData& parent, MethodIdentifier method);

			// Commits the change of a newly created, or updated `rep` root value into
			// this cord. `old_rep` indicates the old (inlined or tree) value of the
			// cord, and determines if the commit invokes SetTree() or EmplaceTree().
			void CommitTree(absl::Nullable<const CordRep*> old_rep, absl::Nonnull<CordRep*> rep, const CordzUpdateScope& scope, MethodIdentifier method);

			void AppendTreeToInlined(absl::Nonnull<CordRep*> tree, MethodIdentifier method);
			void AppendTreeToTree(absl::Nonnull<CordRep*> tree, MethodIdentifier method);
			void AppendTree(absl::Nonnull<CordRep*> tree, MethodIdentifier method);
			void PrependTreeToInlined(absl::Nonnull<CordRep*> tree, MethodIdentifier method);
			void PrependTreeToTree(absl::Nonnull<CordRep*> tree, MethodIdentifier method);
		...
		static bool GetFlatAux(absl::Nonnull<absl::cord_internal::CordRep*> rep, absl::Nonnull<absl::string_view*> fragment);

		// Helper for ForEachChunk().
		static void ForEachChunkAux(absl::Nonnull<absl::cord_internal::CordRep*> rep, absl::FunctionRef<void(absl::string_view)> callback);
		...
		template<typename ResultType, typename RHS> friend ResultType GenericCompare(const Cord& lhs, const RHS& rhs, size_t size_to_compare);
		...
		CordBuffer GetAppendBufferSlowPath(size_t block_size, size_t capacity, size_t min_capacity);
		...
		absl::Nullable<const crc_internal::CrcCordState*> MaybeGetCrcCordState() const;
...
namespace absl {
	...
	namespace cord_internal {
		...
		absl::Nonnull<CordRep*> NewExternalRep(absl::string_view data, Releaser&& releaser) {
			...
			CordRepExternal* rep = new CordRepExternalImpl<ReleaserType>(std::forward<Releaser>(releaser), 0);
		...
		// NOLINTNEXTLINE - suppress clang-tidy raw pointer return.
		inline absl::Nonnull<CordRep*> NewExternalRep(absl::string_view data, void (&releaser)(absl::string_view)) {
			return NewExternalRep(data, &releaser);
		}

	}// namespace cord_internal

	template<typename Releaser> Cord MakeCordFromExternal(absl::string_view data, Releaser&& releaser) {
		Cord cord;
		if (ABSL_PREDICT_TRUE(!data.empty())) {
			...
			using ReleaserType = absl::decay_t<Releaser>;
			cord_internal::InvokeReleaser(cord_internal::Rank1{}, ReleaserType(std::forward<Releaser>(releaser)), data);
		}
		return cord;
	}

	constexpr Cord::InlineRep::InlineRep(absl::string_view sv, absl::Nullable<CordRep*> rep) : data_(sv, rep) {
	}

	inline Cord::InlineRep::InlineRep(const Cord::InlineRep& src) : data_(InlineData::kDefaultInit) {
	...
	inline absl::Nonnull<absl::cord_internal::CordRep*> Cord::InlineRep::as_tree() const {
		assert(data_.is_tree());
		return data_.as_tree();
	}

	inline absl::Nullable<absl::cord_internal::CordRep*> Cord::InlineRep::tree() const {
		if (is_tree()) {
			return as_tree();
		} else {
			return nullptr;
	...
	inline absl::Nonnull<cord_internal::CordRepFlat*> Cord::InlineRep::MakeFlatWithExtraCapacity(size_t extra) {
	...
	inline void Cord::InlineRep::EmplaceTree(absl::Nonnull<CordRep*> rep, MethodIdentifier method) {
		assert(rep);
		data_.make_tree(rep);
		CordzInfo::MaybeTrackCord(data_, method);
	}

	inline void Cord::InlineRep::EmplaceTree(absl::Nonnull<CordRep*> rep, const InlineData& parent, MethodIdentifier method) {
		data_.make_tree(rep);
		CordzInfo::MaybeTrackCord(data_, parent, method);
	}

	inline void Cord::InlineRep::SetTree(absl::Nonnull<CordRep*> rep, const CordzUpdateScope& scope) {
		assert(rep);
		assert(data_.is_tree());
		data_.set_tree(rep);
	...
	inline void Cord::InlineRep::SetTreeOrEmpty(absl::Nullable<CordRep*> rep, const CordzUpdateScope& scope) {
		assert(data_.is_tree());
		if (rep) {
			data_.set_tree(rep);
		} else {
	...
	inline void Cord::InlineRep::CommitTree(absl::Nullable<const CordRep*> old_rep, absl::Nonnull<CordRep*> rep, const CordzUpdateScope& scope, MethodIdentifier method) {
		if (old_rep) {
			SetTree(rep, scope);
		} else {
	...
	inline Cord::Cord(absl::string_view src) : Cord(src, CordzUpdateTracker::kConstructorString) {
	}

	template<typename T> constexpr Cord::Cord(strings_internal::StringConstant<T>)
		: contents_(strings_internal::StringConstant<T>::value,
			  strings_internal::StringConstant<T>::value.size() <= cord_internal::kMaxInline
				  ? nullptr
				  : &cord_internal::ConstInitExternalStorage<strings_internal::StringConstant<T>>::value) {
	}
	...
	inline size_t Cord::EstimatedMemoryUsage(CordMemoryAccounting accounting_method) const {
	...
	inline absl::optional<absl::string_view> Cord::TryFlat() const ABSL_ATTRIBUTE_LIFETIME_BOUND {
	...
	inline CordBuffer Cord::GetCustomAppendBuffer(size_t block_size, size_t capacity, size_t min_capacity) {
		if (empty()) {
	...
	inline void Cord::ChunkIterator::InitTree(absl::Nonnull<cord_internal::CordRep*> tree) {
	...
	inline Cord::ChunkIterator::ChunkIterator(absl::Nonnull<cord_internal::CordRep*> tree) {
	...
	inline Cord Cord::AdvanceAndRead(absl::Nonnull<CharIterator*> it, size_t n_bytes) {
		assert(it != nullptr);
