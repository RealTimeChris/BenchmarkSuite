...
namespace absl {
	...
	#if ABSL_HAVE_BUILTIN(__builtin_memcmp) || (defined(__GNUC__) && !defined(__clang__)) || (defined(_MSC_VER) && _MSC_VER >= 1928)
...
namespace absl {
	...
	class ABSL_ATTRIBUTE_VIEW string_view {
	  public:
		...
		template<typename Allocator> string_view(// NOLINT(runtime/explicit)
		...
		constexpr string_view(absl::Nullable<const char*> data, size_type len) : ptr_(data), length_(CheckLengthInternal(len)) {
		}
		...
		constexpr int compare(size_type pos1, size_type count1, absl::Nonnull<const char*> s) const {
			return substr(pos1, count1).compare(string_view(s));
		}

		// Overload of `string_view::compare()` for comparing a substring of the
		// `string_view` and a substring of a different C-style string `s`.
		constexpr int compare(size_type pos1, size_type count1, absl::Nonnull<const char*> s, size_type count2) const {
		...
		// from `//../strings/match.h` for compatibility.
		constexpr bool starts_with(string_view s) const noexcept {
			return s.empty() || (size() >= s.size() && ABSL_INTERNAL_STRING_VIEW_MEMCMP(data(), s.data(), s.size()) == 0);
		}
		...
		constexpr bool ends_with(string_view s) const noexcept {
			return s.empty() || (size() >= s.size() && ABSL_INTERNAL_STRING_VIEW_MEMCMP(data() + (size() - s.size()), s.data(), s.size()) == 0);
		}
	  ...
	  private:
		...
		string_view(absl::Nullable<const char*> data, size_type len, SkipCheckLengthTag) noexcept : ptr_(data), length_(len) {
		}
		...
		static constexpr size_type StrlenInternal(absl::Nonnull<const char*> str) {
			...
			while (*str != '\0')
				++str;
			return str - begin;
	#elif ABSL_HAVE_BUILTIN(__builtin_strlen) || (defined(__GNUC__) && !defined(__clang__))
		...
		static constexpr int CompareImpl(size_type length_a, size_type length_b, int compare_result) {
			return compare_result == 0 ? static_cast<int>(length_a > length_b) - static_cast<int>(length_a < length_b) : (compare_result < 0 ? -1 : 1);
		}
	...
	constexpr bool operator==(string_view x, string_view y) noexcept {
		return x.size() == y.size() && (x.empty() || ABSL_INTERNAL_STRING_VIEW_MEMCMP(x.data(), y.data(), x.size()) == 0);
	}
