...
#include "../strings/internal/cordz_sample_token.h"
#include "../strings/internal/cordz_statistics.h"
...
namespace absl {
	...
	inline absl::Nullable<const cord_internal::CordzInfo*> GetCordzInfoForTesting(const Cord& cord) {
	...
	inline bool CordzInfoIsListed(absl::Nonnull<const cord_internal::CordzInfo*> cordz_info, cord_internal::CordzSampleToken token = {}) {
	...
	MATCHER_P(HasValidCordzInfoOf, method, "CordzInfo matches cord") {
		...
		if (stat.size != arg.size()) {
			*result_listener << "cordz size " << stat.size << " does not match cord size " << arg.size();
			return false;
		}
		if (stat.update_tracker.Value(method) != 1) {
			*result_listener << "Expected method count 1 for " << method << ", found " << stat.update_tracker.Value(method);
			return false;
		}
		return true;
	}
	...
	MATCHER_P2(CordzMethodCountEq, method, n, absl::StrCat("CordzInfo method count equals ", n)) {
		...
		if (stat.update_tracker.Value(method) != n) {
			*result_listener << "Expected method count " << n << " for " << method << ", found " << stat.update_tracker.Value(method);
			return false;
		}
		return true;
	}
	...
	class CordzSamplingIntervalHelper {
	  public:
		explicit CordzSamplingIntervalHelper(int32_t interval) : orig_mean_interval_(absl::cord_internal::get_cordz_mean_interval()) {
