...
//
// NOTE: Use of `AlphaNum` outside of the //../strings package is unsupported
...
#include "../strings/internal/resize_uninitialized.h"
...
namespace absl {
	...
	struct Hex {
		...
		template<typename Int> explicit Hex(Int v, PadSpec spec = absl::kNoPad, typename std::enable_if<sizeof(Int) == 1 && !std::is_pointer<Int>::value>::type* = nullptr)
			: Hex(spec, static_cast<uint8_t>(v)) {
		}
		template<typename Int> explicit Hex(Int v, PadSpec spec = absl::kNoPad, typename std::enable_if<sizeof(Int) == 2 && !std::is_pointer<Int>::value>::type* = nullptr)
			: Hex(spec, static_cast<uint16_t>(v)) {
		}
		template<typename Int> explicit Hex(Int v, PadSpec spec = absl::kNoPad, typename std::enable_if<sizeof(Int) == 4 && !std::is_pointer<Int>::value>::type* = nullptr)
			: Hex(spec, static_cast<uint32_t>(v)) {
		}
		template<typename Int> explicit Hex(Int v, PadSpec spec = absl::kNoPad, typename std::enable_if<sizeof(Int) == 8 && !std::is_pointer<Int>::value>::type* = nullptr)
			: Hex(spec, static_cast<uint64_t>(v)) {
		}
		template<typename Pointee> explicit Hex(absl::Nullable<Pointee*> v, PadSpec spec = absl::kNoPad) : Hex(spec, reinterpret_cast<uintptr_t>(v)) {
		}
		...
		template<typename S> friend void AbslStringify(S& sink, Hex hex) {
			...
			auto real_width = absl::numbers_internal::FastHexToBufferZeroPad16(hex.value, end - 16);
	  ...
	  private:
		Hex(PadSpec spec, uint64_t v)
			: value(v), width(spec == absl::kNoPad		   ? 1
	...
	struct Dec {
		...
		template<typename Int> explicit Dec(Int v, PadSpec spec = absl::kNoPad, typename std::enable_if<(sizeof(Int) <= 8)>::type* = nullptr)
			: value(v >= 0 ? static_cast<uint64_t>(v) : uint64_t{ 0 } - static_cast<uint64_t>(v)), width(spec == absl::kNoPad		  ? 1
																										   : spec >= absl::kSpacePad2 ? spec - absl::kSpacePad2 + 2
																																	  : spec - absl::kZeroPad2 + 2),
	...
	class AlphaNum {
	  public:
		...
		template<typename T> AlphaNum(std::initializer_list<T>) = delete;// NOLINT(runtime/explicit)
		...
		template<size_t size> AlphaNum(// NOLINT(runtime/explicit)
			const strings_internal::AlphaNumBuffer<size>& buf ABSL_ATTRIBUTE_LIFETIME_BOUND)
		...
		template<typename T, typename = typename std::enable_if<HasAbslStringify<T>::value>::type> AlphaNum(// NOLINT(runtime/explicit)
			const T& v ABSL_ATTRIBUTE_LIFETIME_BOUND, strings_internal::StringifySink&& sink ABSL_ATTRIBUTE_LIFETIME_BOUND = {})
			: piece_(strings_internal::ExtractStringification(sink, v)) {
		}

		template<typename Allocator> AlphaNum(// NOLINT(runtime/explicit)
			const std::basic_string<char, std::char_traits<char>, Allocator>& str ABSL_ATTRIBUTE_LIFETIME_BOUND)
			: piece_(str) {
		}
		...
		template<typename T,
			typename std::enable_if<std::is_enum<T>{} && !std::is_convertible<T, int>{} && !HasAbslStringify<T>::value,
				char*>::type = nullptr>
		AlphaNum(T e)// NOLINT(runtime/explicit)
		...
		template<typename T,
			typename std::enable_if<std::is_class<T>::value &&
				(std::is_same<T, std::vector<bool>::reference>::value || std::is_same<T, std::vector<bool>::const_reference>::value)>::type* = nullptr>
	...
	namespace strings_internal {
		...
		template<typename Integer> std::string IntegerToString(Integer i) {
			...
			assert((size < result.size()) && "StrCat(Integer) does not fit into kMaxDigits10");
			result.erase(size);
			return result;
		}
		template<typename Float> std::string FloatToString(Float f) {
			std::string result;
			strings_internal::STLStringResizeUninitialized(&result, numbers_internal::kSixDigitsToBufferSize);
			char* start = &result[0];
...
// libstc++ or MSSTL.
	...
	#define ABSL_INTERNAL_STRCAT_ENABLE_FAST_CASE false
		...
		template<typename T, typename = std::enable_if_t<ABSL_INTERNAL_STRCAT_ENABLE_FAST_CASE && std::is_arithmetic<T>{} && !std::is_same<T, char>{}>> using EnableIfFastCase = T;
...
#undef ABSL_INTERNAL_STRCAT_ENABLE_FAST_CASE
	...
	template<typename T> ABSL_MUST_USE_RESULT inline std::string StrCat(strings_internal::EnableIfFastCase<T> a) {
	...
	ABSL_MUST_USE_RESULT inline std::string StrCat(const AlphaNum& a, const AlphaNum& b, const AlphaNum& c, const AlphaNum& d, const AlphaNum& e, const AV&... args) {
		return strings_internal::CatPieces({ a.Piece(), b.Piece(), c.Piece(), d.Piece(), e.Piece(), static_cast<const AlphaNum&>(args).Piece()... });
	}
	...
	// Support 5 or more arguments
	template<typename... AV>
	inline void StrAppend(absl::Nonnull<std::string*> dest, const AlphaNum& a, const AlphaNum& b, const AlphaNum& c, const AlphaNum& d, const AlphaNum& e, const AV&... args) {
		strings_internal::AppendPieces(dest, { a.Piece(), b.Piece(), c.Piece(), d.Piece(), e.Piece(), static_cast<const AlphaNum&>(args).Piece()... });
	}
