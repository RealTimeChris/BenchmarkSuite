...
namespace absl {
	...
	inline constexpr bool StartsWith(absl::string_view text, absl::string_view prefix) noexcept {
		if (prefix.empty()) {
			return true;
		}
	...
	inline constexpr bool EndsWith(absl::string_view text, absl::string_view suffix) noexcept {
		if (suffix.empty()) {
			return true;
		}
