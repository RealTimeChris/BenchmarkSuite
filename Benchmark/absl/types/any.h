...
namespace absl {
	...
	#include "../base/internal/fast_type_id.h"
	#include "../meta/type_traits.h"
	#include "../types/bad_any_cast.h"
...
namespace absl {
	...
	// initializer list.
	template<typename T, typename U, typename... Args> any make_any(std::initializer_list<U> il, Args&&... args);
	...
	// match the cast.
	template<typename ValueType> ValueType any_cast(any& operand);// NOLINT(runtime/references)
	...
	template<typename ValueType> const ValueType* any_cast(const any* operand) noexcept;
	...
	class any {
	  ...
	  public:
		...
		// `false`.
		any(const any& other) : obj_(other.has_value() ? other.obj_->Clone() : std::unique_ptr<ObjInterface>()) {
		}
		...
		template<typename T, typename VT = absl::decay_t<T>,
			absl::enable_if_t<!absl::disjunction<std::is_same<any, VT>, IsInPlaceType<VT>, absl::negation<std::is_copy_constructible<VT>>>::value>* = nullptr>
		...
		template<typename T, typename... Args, typename VT = absl::decay_t<T>,
			absl::enable_if_t<absl::conjunction<std::is_copy_constructible<VT>, std::is_constructible<VT, Args...>>::value>* = nullptr>
		...
		template<typename T, typename U, typename... Args, typename VT = absl::decay_t<T>,
			absl::enable_if_t<absl::conjunction<std::is_copy_constructible<VT>, std::is_constructible<VT, std::initializer_list<U>&, Args...>>::value>* = nullptr>
		explicit any(in_place_type_t<T> /*tag*/, std::initializer_list<U> ilist, Args&&... args) : obj_(new Obj<VT>(in_place, ilist, std::forward<Args>(args)...)) {
		}
		...
		template<typename T, typename VT = absl::decay_t<T>,
			absl::enable_if_t<absl::conjunction<absl::negation<std::is_same<VT, any>>, std::is_copy_constructible<VT>>::value>* = nullptr>
		any& operator=(T&& rhs) {
		...
		VT& emplace(std::initializer_list<U> ilist, Args&&... args) ABSL_ATTRIBUTE_LIFETIME_BOUND {
			reset();// NOTE: reset() is required here even in the world of exceptions.
	  ...
	  private:
		...
		template<typename T> class Obj : public ObjInterface {
		  public:
			template<typename... Args> explicit Obj(in_place_t /*tag*/, Args&&... args) : value(std::forward<Args>(args)...) {
			}
		...
		// Description at the declaration site (top of file).
		template<typename ValueType> friend ValueType any_cast(any& operand);// NOLINT(runtime/references)
	...
	template<typename T> struct any::IsInPlaceType<in_place_type_t<T>> : std::true_type {};
	...
	template<typename T> const T* any_cast(const any* operand) noexcept {
		...
		return operand && operand->GetObjTypeId() == any::IdForType<U>() ? std::addressof(static_cast<const any::Obj<U>*>(operand->obj_.get())->value) : nullptr;
	}

	// Description at the declaration site (top of file).
	template<typename T> T* any_cast(any* operand) noexcept {
		using U = typename std::remove_cv<typename std::remove_reference<T>::type>::type;
		return operand && operand->GetObjTypeId() == any::IdForType<U>() ? std::addressof(static_cast<any::Obj<U>*>(operand->obj_.get())->value) : nullptr;
	}
