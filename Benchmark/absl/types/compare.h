...
namespace absl {
	...
	// incomplete types so they need to be defined after the types are complete.
		...
		#define ABSL_COMPARE_INLINE_INIT(type, name, init) inline constexpr type type::name(init)

	#else// __cpp_inline_variables

		#define ABSL_COMPARE_INLINE_BASECLASS_DECL(name) ABSL_CONST_INIT static const T name

		// A no-op expansion that can be followed by a semicolon at class level.
		#define ABSL_COMPARE_INLINE_SUBCLASS_DECL(type, name) static_assert(true, "")

		#define ABSL_COMPARE_INLINE_INIT(type, name, init) template<typename T> const T compare_internal::type##_base<T>::name(init)
	...
	namespace compare_internal {
		...
		template<typename BoolT, absl::enable_if_t<std::is_same<bool, BoolT>::value, int> = 0> constexpr bool compare_result_as_less_than(const BoolT r) {
			return r;
		}
		...
		template<typename Compare, typename K, typename LK> constexpr bool do_less_than_comparison(const Compare& compare, const K& x, const LK& y) {
		...
		template<typename Int, absl::enable_if_t<std::is_same<int, Int>::value, int> = 0> constexpr absl::weak_ordering compare_result_as_ordering(const Int c) {
			return c < 0 ? absl::weak_ordering::less : c == 0 ? absl::weak_ordering::equivalent : absl::weak_ordering::greater;
		}
		...
		template<typename Compare, typename K, typename LK, absl::enable_if_t<std::is_same<bool, absl::result_of_t<Compare(const K&, const LK&)>>::value, int> = 0>
		constexpr absl::weak_ordering do_three_way_comparison(const Compare& compare, const K& x, const LK& y) {
			return compare(x, y) ? absl::weak_ordering::less : compare(y, x) ? absl::weak_ordering::greater : absl::weak_ordering::equivalent;
		}
