...
#include "../container/internal/raw_hash_set.h"// IWYU pragma: export
...
namespace absl {
	...
	namespace container_internal {
		template<typename T> struct NodeHashSetPolicy;
	}// namespace container_internal
	...
	//  }
	template<class T, class Hash = DefaultHashContainerHash<T>, class Eq = DefaultHashContainerEq<T>, class Alloc = std::allocator<T>> class ABSL_ATTRIBUTE_OWNER node_hash_set
	...
	typename node_hash_set<T, H, E, A>::size_type erase_if(node_hash_set<T, H, E, A>& c, Predicate pred) {
	...
	template<typename T, typename H, typename E, typename A> void swap(node_hash_set<T, H, E, A>& x, node_hash_set<T, H, E, A>& y) noexcept(noexcept(x.swap(y))) {
		return x.swap(y);
	}
	...
	namespace container_internal {
		...
		template<typename T, typename H, typename E, typename A, typename Function> decay_t<Function> c_for_each_fast(const node_hash_set<T, H, E, A>& c, Function&& f) {
			container_internal::ForEach(f, &c);
			return f;
		}
		template<typename T, typename H, typename E, typename A, typename Function> decay_t<Function> c_for_each_fast(node_hash_set<T, H, E, A>& c, Function&& f) {
			container_internal::ForEach(f, &c);
			return f;
		}
		template<typename T, typename H, typename E, typename A, typename Function> decay_t<Function> c_for_each_fast(node_hash_set<T, H, E, A>&& c, Function&& f) {
	...
	namespace container_internal {
		...
		template<class T> struct NodeHashSetPolicy : absl::container_internal::node_slot_policy<T&, NodeHashSetPolicy<T>> {
			using key_type			 = T;
			using init_type			 = T;
			using constant_iterators = std::true_type;

			template<class Allocator, class... Args> static T* new_element(Allocator* alloc, Args&&... args) {
				using ValueAlloc = typename absl::allocator_traits<Allocator>::template rebind_alloc<T>;
			...
			template<class Allocator> static void delete_element(Allocator* alloc, T* elem) {
				using ValueAlloc = typename absl::allocator_traits<Allocator>::template rebind_alloc<T>;
				ValueAlloc value_alloc(*alloc);
			...
			template<class F, class... Args> static decltype(absl::container_internal::DecomposeValue(std::declval<F>(), std::declval<Args>()...)) apply(F&& f, Args&&... args) {
				return absl::container_internal::DecomposeValue(std::forward<F>(f), std::forward<Args>(args)...);
			}
	...
	namespace container_algorithm_internal {
		...
		template<class Key, class Hash, class KeyEqual, class Allocator> struct IsUnorderedContainer<absl::node_hash_set<Key, Hash, KeyEqual, Allocator>> : std::true_type {};
