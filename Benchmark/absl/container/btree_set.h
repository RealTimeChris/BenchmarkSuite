...
#include "../container/internal/btree.h"// IWYU pragma: export
#include "../container/internal/btree_container.h"// IWYU pragma: export
...
namespace absl {
	...
	namespace container_internal {
		...
		template<typename Key> struct set_slot_policy;

		template<typename Key, typename Compare, typename Alloc, int TargetNodeSize, bool IsMulti> struct set_params;
	...
	//
	template<typename Key, typename Compare = std::less<Key>, typename Alloc = std::allocator<Key>> class ABSL_ATTRIBUTE_OWNER btree_set
		: public container_internal::btree_set_container<container_internal::btree<container_internal::set_params<Key, Compare, Alloc, /*TargetNodeSize=*/256,
			  /*IsMulti=*/false>>> {
	...
	template<typename K, typename C, typename A> void swap(btree_set<K, C, A>& x, btree_set<K, C, A>& y) {
		return x.swap(y);
	}
	...
	template<typename K, typename C, typename A, typename Pred> typename btree_set<K, C, A>::size_type erase_if(btree_set<K, C, A>& set, Pred pred) {
	...
	//
	template<typename Key, typename Compare = std::less<Key>, typename Alloc = std::allocator<Key>> class ABSL_ATTRIBUTE_OWNER btree_multiset
		: public container_internal::btree_multiset_container<container_internal::btree<container_internal::set_params<Key, Compare, Alloc, /*TargetNodeSize=*/256,
			  /*IsMulti=*/true>>> {
	...
	template<typename K, typename C, typename A> void swap(btree_multiset<K, C, A>& x, btree_multiset<K, C, A>& y) {
		return x.swap(y);
	}
	...
	// Erases all elements that satisfy the predicate pred from the container.
	// Returns the number of erased elements.
	template<typename K, typename C, typename A, typename Pred> typename btree_multiset<K, C, A>::size_type erase_if(btree_multiset<K, C, A>& set, Pred pred) {
		return container_internal::btree_access::erase_if(set, std::move(pred));
	}
	...
	namespace container_internal {
		...
		template<typename Key> struct set_slot_policy {
			...
			template<typename Alloc, class... Args> static void construct(Alloc* alloc, slot_type* slot, Args&&... args) {
				absl::allocator_traits<Alloc>::construct(*alloc, slot, std::forward<Args>(args)...);
			}

			template<typename Alloc> static void construct(Alloc* alloc, slot_type* slot, slot_type* other) {
				absl::allocator_traits<Alloc>::construct(*alloc, slot, std::move(*other));
			}

			template<typename Alloc> static void construct(Alloc* alloc, slot_type* slot, const slot_type* other) {
				absl::allocator_traits<Alloc>::construct(*alloc, slot, *other);
			}

			template<typename Alloc> static void destroy(Alloc* alloc, slot_type* slot) {
		...
		template<typename Key, typename Compare, typename Alloc, int TargetNodeSize, bool IsMulti> struct set_params : common_params<Key, Compare, Alloc, TargetNodeSize, IsMulti,
