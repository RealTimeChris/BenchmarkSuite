...
namespace absl {
	...
	namespace container_internal {
		template<class K, class V> struct FlatHashMapPolicy;
	}// namespace container_internal
	...
	//  }
	template<class K, class V, class Hash = DefaultHashContainerHash<K>, class Eq = DefaultHashContainerEq<K>, class Allocator = std::allocator<std::pair<const K, V>>>
	class ABSL_ATTRIBUTE_OWNER flat_hash_map : public absl::container_internal::raw_hash_map<absl::container_internal::FlatHashMapPolicy<K, V>, Hash, Eq, Allocator> {
	...
	template<typename K, typename V, typename H, typename E, typename A> void swap(flat_hash_map<K, V, H, E, A>& x, flat_hash_map<K, V, H, E, A>& y) noexcept(noexcept(x.swap(y))) {
		x.swap(y);
	}
	...
	namespace container_internal {
		...
		template<typename K, typename V, typename H, typename E, typename A, typename Function> decay_t<Function> c_for_each_fast(flat_hash_map<K, V, H, E, A>& c, Function&& f) {
			container_internal::ForEach(f, &c);
			return f;
		}
		template<typename K, typename V, typename H, typename E, typename A, typename Function> decay_t<Function> c_for_each_fast(flat_hash_map<K, V, H, E, A>&& c, Function&& f) {
	...
	namespace container_internal {
		...
		template<class K, class V> struct FlatHashMapPolicy {
			...
			template<class Allocator> static auto transfer(Allocator* alloc, slot_type* new_slot, slot_type* old_slot) {
			...
			template<class Hash> static constexpr HashSlotFn get_hash_slot_fn() {
				return memory_internal::IsLayoutCompatible<K, V>::value ? &TypeErasedApplyToSlotFn<Hash, K> : nullptr;
			}
	...
	namespace container_algorithm_internal {
		...
		template<class Key, class T, class Hash, class KeyEqual, class Allocator> struct IsUnorderedContainer<absl::flat_hash_map<Key, T, Hash, KeyEqual, Allocator>>
			: std::true_type {};
