...
namespace absl {
	...
	namespace container_internal {
		...
		template<typename T, typename U> struct remove_pair_const<std::pair<T, U>> {
			using type = std::pair<typename remove_pair_const<T>::type, typename remove_pair_const<U>::type>;
		};

		// Utility class to provide an accessor for a key given a value. The default
		// behavior is to treat the value as a pair and return the first element.
		template<typename K, typename V> struct KeyOfValue {
			struct type {
		...
		template<typename K> struct KeyOfValue<K, K> {
			struct type {
		...
		template<typename K> struct Generator {
			int maxval;
		...
		template<> struct Generator<absl::Time> {
			int maxval;
			explicit Generator(int m) : maxval(m) {
			}
		...
		template<> struct Generator<Cord> {
			int maxval;
			explicit Generator(int m) : maxval(m) {
			}
		...
		template<typename V> std::vector<V> GenerateValuesWithSeed(int n, int maxval, int seed) {
