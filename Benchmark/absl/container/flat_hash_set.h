...
namespace absl {
	...
	namespace container_internal {
		template<typename T> struct FlatHashSetPolicy;
	}// namespace container_internal
	...
	//  }
	template<class T, class Hash = DefaultHashContainerHash<T>, class Eq = DefaultHashContainerEq<T>, class Allocator = std::allocator<T>> class ABSL_ATTRIBUTE_OWNER flat_hash_set
		: public absl::container_internal::raw_hash_set<absl::container_internal::FlatHashSetPolicy<T>, Hash, Eq, Allocator> {
	...
	typename flat_hash_set<T, H, E, A>::size_type erase_if(flat_hash_set<T, H, E, A>& c, Predicate pred) {
	...
	template<typename T, typename H, typename E, typename A> void swap(flat_hash_set<T, H, E, A>& x, flat_hash_set<T, H, E, A>& y) noexcept(noexcept(x.swap(y))) {
		return x.swap(y);
	}
	...
	namespace container_internal {
		...
		template<typename T, typename H, typename E, typename A, typename Function> decay_t<Function> c_for_each_fast(const flat_hash_set<T, H, E, A>& c, Function&& f) {
			container_internal::ForEach(f, &c);
			return f;
		}
		template<typename T, typename H, typename E, typename A, typename Function> decay_t<Function> c_for_each_fast(flat_hash_set<T, H, E, A>& c, Function&& f) {
			container_internal::ForEach(f, &c);
			return f;
		}
		template<typename T, typename H, typename E, typename A, typename Function> decay_t<Function> c_for_each_fast(flat_hash_set<T, H, E, A>&& c, Function&& f) {
	...
	namespace container_internal {
		...
		template<class T> struct FlatHashSetPolicy {
			using slot_type			 = T;
			using key_type			 = T;
			using init_type			 = T;
			...
			template<class Allocator> static auto destroy(Allocator* alloc, slot_type* slot) {
	...
	namespace container_algorithm_internal {
		...
		template<class Key, class Hash, class KeyEqual, class Allocator> struct IsUnorderedContainer<absl::flat_hash_set<Key, Hash, KeyEqual, Allocator>> : std::true_type {};
