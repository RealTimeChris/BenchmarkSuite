...
namespace absl {
	...
	// otherwise).
	template<typename T> struct is_function : std::integral_constant<bool, !(std::is_reference<T>::value || std::is_const<typename std::add_const<T>::type>::value)> {};
...
//
	...
	#define ABSL_META_INTERNAL_STD_HASH_SFINAE_FRIENDLY_ 1
		...
		struct IsHashable<Key, absl::enable_if_t<std::is_convertible<decltype(std::declval<std::hash<Key>&>()(std::declval<Key const&>())), std::size_t>::value>> : std::true_type {
		};
	...
	namespace swap_internal {
		...
		template<class T, class IsNoexcept = std::integral_constant<bool, noexcept(swap(std::declval<T&>(), std::declval<T&>()))>> using IsNothrowSwappableImpl =
...
#if ABSL_HAVE_BUILTIN(__is_trivially_relocatable) && (defined(__cpp_impl_trivially_relocatable) || (!defined(__clang__) && !defined(__APPLE__) && !defined(__NVCC__)))
	template<class T> struct is_trivially_relocatable : std::integral_constant<bool, __is_trivially_relocatable(T)> {};
#elif ABSL_HAVE_BUILTIN(__is_trivially_relocatable) && defined(__clang__) && !(defined(_WIN32) || defined(_WIN64)) && !defined(__APPLE__) && !defined(__NVCC__)
	template<class T> struct is_trivially_relocatable
		: std::integral_constant<bool, std::is_trivially_copyable<T>::value || (__is_trivially_relocatable(T) && std::is_trivially_move_assignable<T>::value)> {};
#else
...
//
	...
	namespace type_traits_internal {
		...
		template<typename T> struct IsOwnerImpl<T, std::enable_if_t<std::is_class<typename T::absl_internal_is_view>::value>> : absl::negation<typename T::absl_internal_is_view> {
		};
		...
		template<typename T, typename U> using IsLifetimeBoundAssignment =
			absl::conjunction<std::integral_constant<bool, !std::is_lvalue_reference<U>::value>, IsOwner<absl::remove_cvref_t<U>>, IsView<absl::remove_cvref_t<T>>>;
