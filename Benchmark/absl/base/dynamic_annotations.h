...
#include "../base/internal/dynamic_annotations.h"// IWYU pragma: export
...
// Define race annotations.
	...
	#define ABSL_ANNOTATE_THREAD_NAME(name) ABSL_INTERNAL_GLOBAL_SCOPED(AnnotateThreadName)(__FILE__, __LINE__, name)
	...
	#define ABSL_ANNOTATE_RWLOCK_CREATE(lock) ABSL_INTERNAL_GLOBAL_SCOPED(AnnotateRWLockCreate)(__FILE__, __LINE__, lock)
	...
	#define ABSL_ANNOTATE_RWLOCK_DESTROY(lock) ABSL_INTERNAL_GLOBAL_SCOPED(AnnotateRWLockDestroy)(__FILE__, __LINE__, lock)
...
// Define memory annotations.
	...
	#define ABSL_ANNOTATE_MEMORY_IS_INITIALIZED(address, size) __msan_unpoison(address, size)

	#define ABSL_ANNOTATE_MEMORY_IS_UNINITIALIZED(address, size) __msan_allocated_memory(address, size)
...
// Define IGNORE_READS_BEGIN/_END attributes.
	...
	#define ABSL_INTERNAL_IGNORE_READS_BEGIN_ATTRIBUTE __attribute((exclusive_lock_function("*")))
	#define ABSL_INTERNAL_IGNORE_READS_END_ATTRIBUTE __attribute((unlock_function("*")))
...
// implementation.
ABSL_INTERNAL_BEGIN_EXTERN_C
void AnnotateIgnoreReadsBegin(const char* file, int line) ABSL_INTERNAL_IGNORE_READS_BEGIN_ATTRIBUTE;
...
ABSL_INTERNAL_END_EXTERN_C
	...
	#define ABSL_ANNOTATE_IGNORE_READS_BEGIN() \
		ABSL_INTERNAL_GLOBAL_SCOPED(ABSL_INTERNAL_C_SYMBOL(AbslInternalAnnotateIgnoreReadsBegin)) \
		()

	#define ABSL_ANNOTATE_IGNORE_READS_END() \
		ABSL_INTERNAL_GLOBAL_SCOPED(ABSL_INTERNAL_C_SYMBOL(AbslInternalAnnotateIgnoreReadsEnd)) \
		()
...
// Define IGNORE_WRITES_BEGIN/_END annotations.
	...
	#define ABSL_ANNOTATE_IGNORE_WRITES_BEGIN() ABSL_INTERNAL_GLOBAL_SCOPED(AnnotateIgnoreWritesBegin)(__FILE__, __LINE__)

	// Stop ignoring writes.
	#define ABSL_ANNOTATE_IGNORE_WRITES_END() ABSL_INTERNAL_GLOBAL_SCOPED(AnnotateIgnoreWritesEnd)(__FILE__, __LINE__)
...
//        ... = ABSL_ANNOTATE_UNPROTECTED_READ(x);
	...
	#define ABSL_ANNOTATE_IGNORE_READS_AND_WRITES_END() \
		...
		#define ABSL_ANNOTATE_UNPROTECTED_READ(x) absl::base_internal::AnnotateUnprotectedRead(x)

namespace absl {
	ABSL_NAMESPACE_BEGIN
	namespace base_internal {

		template<typename T> inline T AnnotateUnprotectedRead(const volatile T& x) {// NOLINT
			ABSL_ANNOTATE_IGNORE_READS_BEGIN();
			T res = x;
			ABSL_ANNOTATE_IGNORE_READS_END();
			return res;
...
// Address sanitizer annotations
	...
	#define ABSL_ANNOTATE_CONTIGUOUS_CONTAINER(beg, end, old_mid, new_mid) __sanitizer_annotate_contiguous_container(beg, end, old_mid, new_mid)
...
namespace absl {
	...
	#else
	template<typename T> T* HwasanTagPointer(T* ptr, uintptr_t) {
		return ptr;
	}
	#endif
}// namespace absl
