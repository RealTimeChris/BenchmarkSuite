...
#include "../status/internal/status_internal.h"
...
namespace absl {
	...
	class ABSL_ATTRIBUTE_TRIVIAL_ABI Status final {
	  public:
		...
		template<typename Sink> friend void AbslStringify(Sink& sink, const Status& status) {
	  ...
	  private:
		...
		static absl::Nonnull<status_internal::StatusRep*> PrepareToModify(uintptr_t rep);
		...
		static absl::Nonnull<const status_internal::StatusRep*> RepToPointer(uintptr_t r);
	...
	inline Status::Status(const Status& x) : Status(x.rep_) {
		Ref(rep_);
	}
	...
	inline absl::optional<absl::Cord> Status::GetPayload(absl::string_view type_url) const {
		if (IsInlined(rep_))
			return absl::nullopt;
	...
	inline void Status::ForEachPayload(absl::FunctionRef<void(absl::string_view, const absl::Cord&)> visitor) const {
		if (IsInlined(rep_))
			return;
	...
	inline absl::Nonnull<const status_internal::StatusRep*> Status::RepToPointer(uintptr_t rep) {
		assert(!IsInlined(rep));
		return reinterpret_cast<const status_internal::StatusRep*>(rep);
	}

	inline uintptr_t Status::PointerToRep(absl::Nonnull<status_internal::StatusRep*> rep) {
	...
	absl::Nonnull<const char*> StatusMessageAsCStr(const Status& status ABSL_ATTRIBUTE_LIFETIME_BOUND);
