...
#include "../random/internal/mock_helpers.h"
...
namespace absl {
	...
	class MockingBitGen {
	  ...
	  private:
		...
		template<typename ResultT, typename... Args> static auto GetMockFnType(ResultT, std::tuple<Args...>) -> ::testing::MockFunction<ResultT(Args...)>;
		...
		template<typename MockFnType, typename ValidatorT, typename ResultT, typename Tuple> struct MockFnCaller;

		// specialization for std::tuple.
		template<typename MockFnType, typename ValidatorT, typename ResultT, typename... Args> struct MockFnCaller<MockFnType, ValidatorT, ResultT, std::tuple<Args...>> {
			MockFnType* fn;
		...
		template<typename MockFnType, typename ValidatorT, typename ResultT, typename ArgTupleT> class FunctionHolderImpl final : public FunctionHolder {
		  public:
			void Apply(void* args_tuple, void* result) final {
				...
				*static_cast<ResultT*>(result) = absl::apply(MockFnCaller<MockFnType, ValidatorT, ResultT, ArgTupleT>{ &mock_fn_ }, *static_cast<ArgTupleT*>(args_tuple));
			}
		...
		template<typename ResultT, typename ArgTupleT, typename SelfT, typename ValidatorT> auto RegisterMock(SelfT&, base_internal::FastTypeIdType type, ValidatorT)
			...
			using WrappedFnType = absl::conditional_t<std::is_same<SelfT, ::testing::NiceMock<MockingBitGen>>::value, ::testing::NiceMock<MockFnType>,
				absl::conditional_t<std::is_same<SelfT, ::testing::NaggyMock<MockingBitGen>>::value, ::testing::NaggyMock<MockFnType>,
					absl::conditional_t<std::is_same<SelfT, ::testing::StrictMock<MockingBitGen>>::value, ::testing::StrictMock<MockFnType>, MockFnType>>>;

			using ImplT = FunctionHolderImpl<WrappedFnType, ValidatorT, ResultT, ArgTupleT>;
