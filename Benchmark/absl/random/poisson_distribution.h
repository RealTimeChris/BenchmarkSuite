...
#include "../random/internal/fastmath.h"
...
namespace absl {
	...
	// provided by <math.h".
	//
	template<typename IntType = int> class poisson_distribution {
	  public:
	...
	template<typename IntType> poisson_distribution<IntType>::param_type::param_type(double mean) : mean_(mean), split_(0) {
		assert(mean >= 0);
	...
	typename poisson_distribution<IntType>::result_type poisson_distribution<IntType>::operator()(URBG& g,// NOLINT(runtime/references)
		const param_type& p) {
		...
		if (p.split_ != 0) {
			...
			for (int split = p.split_; split > 0; --split) {
				double r = 1.0;
				do {
					r *= GenerateRealFromBits<double, GeneratePositiveTag, true>(fast_u64_(g));// U(-1, 0)
					++n;
				} while (r > p.emu_);
				--n;
			}
		...
		for (;;) {
			const double u = GenerateRealFromBits<double, GeneratePositiveTag, false>(fast_u64_(g));// U(0, 1)
