...
#include "../random/bernoulli_distribution.h"
#include "../random/beta_distribution.h"
#include "../random/exponential_distribution.h"
#include "../random/gaussian_distribution.h"
#include "../random/internal/distribution_caller.h"// IWYU pragma: export
#include "../random/internal/traits.h"
#include "../random/internal/uniform_helper.h"// IWYU pragma: export
#include "../random/log_uniform_int_distribution.h"
#include "../random/poisson_distribution.h"
#include "../random/uniform_int_distribution.h"// IWYU pragma: export
#include "../random/uniform_real_distribution.h"// IWYU pragma: export
#include "../random/zipf_distribution.h"
...
namespace absl {
	...
	//
	template<typename R = void, typename TagType, typename URBG> typename absl::enable_if_t<!std::is_same<R, void>::value, R>//
	Uniform(TagType tag,
	...
	template<typename R = void, typename URBG> typename absl::enable_if_t<!std::is_same<R, void>::value, R>//
	Uniform(URBG&& urbg,// NOLINT(runtime/references)
		...
		return random_internal::DistributionCaller<gen_t>::template Call<distribution_t>(&urbg, lo, hi);
	}
	...
	typename absl::enable_if_t<std::is_same<R, void>::value, random_internal::uniform_inferred_return_t<A, B>> Uniform(TagType tag,
		URBG&& urbg,// NOLINT(runtime/references)
		A lo, B hi) {
		using gen_t			 = absl::decay_t<URBG>;
		...
		return random_internal::DistributionCaller<gen_t>::template Call<distribution_t>(&urbg, tag, static_cast<return_t>(lo), static_cast<return_t>(hi));
	}
	...
	typename absl::enable_if_t<std::is_same<R, void>::value, random_internal::uniform_inferred_return_t<A, B>> Uniform(URBG&& urbg,// NOLINT(runtime/references)
		A lo, B hi) {
		using gen_t			 = absl::decay_t<URBG>;
		using return_t		 = typename random_internal::uniform_inferred_return_t<A, B>;
		using distribution_t = random_internal::UniformDistributionWrapper<return_t>;
	...
	template<typename R, typename URBG> typename absl::enable_if_t<!std::numeric_limits<R>::is_signed, R>//
	Uniform(URBG&& urbg) {// NOLINT(runtime/references)
		using gen_t			 = absl::decay_t<URBG>;
		using distribution_t = random_internal::UniformDistributionWrapper<R>;
	...
	//   }
	//
	template<typename URBG> bool Bernoulli(URBG&& urbg,// NOLINT(runtime/references)
		double p) {
		using gen_t			 = absl::decay_t<URBG>;
	...
	//
	template<typename IntType, typename URBG> IntType Poisson(URBG&& urbg,// NOLINT(runtime/references)
		double mean = 1.0) {
		static_assert(random_internal::IsIntegral<IntType>::value,
			"Template-argument 'IntType' must be an integral type, in "
	...
	template<typename IntType, typename URBG> IntType Zipf(URBG&& urbg,// NOLINT(runtime/references)
		IntType hi = (std::numeric_limits<IntType>::max)(), double q = 2.0, double v = 1.0) {
		static_assert(random_internal::IsIntegral<IntType>::value,
			"Template-argument 'IntType' must be an integral type, in "
