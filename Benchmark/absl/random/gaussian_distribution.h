...
namespace absl {
	...
	namespace random_internal {
		...
		class ABSL_DLL gaussian_distribution_base {
		  public:
			template<typename URBG> inline double zignor(URBG& g);// NOLINT(runtime/references)

		  private:
			friend class TableGenerator;

			template<typename URBG> inline double zignor_fallback(URBG& g,// NOLINT(runtime/references)
				bool neg);
	...
	template<typename RealType = double> class gaussian_distribution : random_internal::gaussian_distribution_base {
	  public:
		...
		class param_type {
		  public:
			...
			explicit param_type(result_type mean = 0, result_type stddev = 1) : mean_(mean), stddev_(stddev) {
			}
		...
		explicit gaussian_distribution(result_type mean, result_type stddev = 1) : param_(mean, stddev) {
		}
	...
	typename gaussian_distribution<RealType>::result_type gaussian_distribution<RealType>::operator()(URBG& g,// NOLINT(runtime/references)
		const param_type& p) {
	...
	namespace random_internal {
		...
		template<typename URBG> inline double gaussian_distribution_base::zignor_fallback(URBG& g, bool neg) {
			...
			do {
				// kRInv = 1/r, U(0, 1)
				x = kRInv * std::log(GenerateRealFromBits<double, GeneratePositiveTag, false>(fast_u64_(g)));
				y = -std::log(GenerateRealFromBits<double, GeneratePositiveTag, false>(fast_u64_(g)));
			} while ((y + y) < (x * x));
			return neg ? (x - kR) : (kR - x);
		}

		template<typename URBG> inline double gaussian_distribution_base::zignor(URBG& g) {// NOLINT(runtime/references)
			using random_internal::GeneratePositiveTag;
			using random_internal::GenerateRealFromBits;
			...
			while (true) {
				...
				double j	   = GenerateRealFromBits<double, GenerateSignedTag, false>(bits);// U(-1, 1)
				...
				double v = GenerateRealFromBits<double, GeneratePositiveTag, false>(fast_u64_(g));// U(0, 1)
				if ((zg_.f[i + 1] + v * (zg_.f[i] - zg_.f[i + 1])) < std::exp(-0.5 * x * x)) {
					return x;
				}
