...
#include "../random/internal/wide_multiply.h"
...
namespace absl {
	...
	template<typename IntType = int> class uniform_int_distribution {
	  ...
	  public:
		...
		class param_type {
		  public:
			...
			explicit param_type(result_type lo = 0, result_type hi = (std::numeric_limits<result_type>::max)())
				: lo_(lo), range_(static_cast<unsigned_type>(hi) - static_cast<unsigned_type>(lo)) {
		...
		explicit uniform_int_distribution(result_type lo, result_type hi = (std::numeric_limits<result_type>::max)()) : param_(lo, hi) {
		}
		...
		template<typename URBG> result_type operator()(URBG& gen) {// NOLINT(runtime/references)
			return (*this)(gen, param());
		}

		template<typename URBG> result_type operator()(URBG& gen, const param_type& param) {// NOLINT(runtime/references)
	  ...
	  private:
		...
		template<typename URBG> unsigned_type Generate(URBG& g,// NOLINT(runtime/references)
			unsigned_type R);
		param_type param_;
	};
	...
	std::basic_ostream<CharT, Traits>& operator<<(std::basic_ostream<CharT, Traits>& os, const uniform_int_distribution<IntType>& x) {
		...
		os << static_cast<stream_type>(x.a()) << os.fill() << static_cast<stream_type>(x.b());
		return os;
	}
	...
	std::basic_istream<CharT, Traits>& operator>>(std::basic_istream<CharT, Traits>& is, uniform_int_distribution<IntType>& x) {
		...
		is >> a >> b;
		if (!is.fail()) {
			x.param(param_type(static_cast<result_type>(a), static_cast<result_type>(b)));
		}
		return is;
	}

	template<typename IntType> template<typename URBG>
	typename random_internal::make_unsigned_bits<IntType>::type uniform_int_distribution<IntType>::Generate(URBG& g,// NOLINT(runtime/references)
		...
		if (ABSL_PREDICT_FALSE(helper::lo(product) < Lim)) {
			...
			// promotion rules.
			const unsigned_type threshold = ((std::numeric_limits<unsigned_type>::max)() - Lim + 1) % Lim;
