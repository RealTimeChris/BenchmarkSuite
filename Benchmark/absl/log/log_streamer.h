...
#include "../log/absl_log.h"
#include "../strings/internal/ostringstream.h"
...
namespace absl {
	...
	class LogStreamer final {
	  public:
		...
		explicit LogStreamer(absl::LogSeverity severity, absl::string_view file, int line) : severity_(severity), line_(line), file_(file), stream_(absl::in_place, &buf_) {
		...
		LogStreamer(LogStreamer&& that) noexcept
			: severity_(that.severity_), line_(that.line_), file_(std::move(that.file_)), buf_(std::move(that.buf_)), stream_(std::move(that.stream_)) {
