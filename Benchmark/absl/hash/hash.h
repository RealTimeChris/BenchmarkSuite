...
namespace absl {
	...
	class HashState : public hash_internal::HashStateBase<HashState> {
	  public:
		...
		template<typename T, absl::enable_if_t<std::is_base_of<hash_internal::HashStateBase<T>, T>::value, int> = 0> static HashState Create(T* state) {
			HashState s;
			s.Init(state);
			return s;
		}
	  ...
	  private:
		...
		template<typename T> static void CombineRawImpl(void* p, uint64_t value) {
			T& state = *static_cast<T*>(p);
		...
		template<typename HS> struct CombineUnorderedInvoker {
			template<typename T, typename ConsumerT> void operator()(T inner_state, ConsumerT inner_cb) {
				f(HashState::Create(&inner_state), [&](HashState& inner_erased) {
