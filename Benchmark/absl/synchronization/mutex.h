...
#include "../base/const_init.h"
#include "../base/internal/identity.h"
#include "../base/internal/low_level_alloc.h"
#include "../base/internal/thread_identity.h"
#include "../base/internal/tsan_mutex_interface.h"
#include "../base/port.h"
#include "../base/thread_annotations.h"
#include "../synchronization/internal/kernel_timeout.h"
#include "../synchronization/internal/per_thread_sem.h"
#include "../time/time.h"
...
namespace absl {
	...
	class ABSL_LOCKABLE ABSL_ATTRIBUTE_WARN_UNUSED Mutex {
	  public:
		...
		// startup and shutdown.  (See ../base/const_init.h.)
		//
		...
		ABSL_MUST_USE_RESULT bool WriterTryLock() ABSL_EXCLUSIVE_TRYLOCK_FUNCTION(true) {
			return this->TryLock();
		}
		...
		void LockWhen(const Condition& cond) ABSL_EXCLUSIVE_LOCK_FUNCTION() {
			LockWhenCommon(cond, synchronization_internal::KernelTimeout::Never(), true);
		}

		void ReaderLockWhen(const Condition& cond) ABSL_SHARED_LOCK_FUNCTION() {
			LockWhenCommon(cond, synchronization_internal::KernelTimeout::Never(), false);
		}
		...
		bool LockWhenWithTimeout(const Condition& cond, absl::Duration timeout) ABSL_EXCLUSIVE_LOCK_FUNCTION() {
			return LockWhenCommon(cond, synchronization_internal::KernelTimeout{ timeout }, true);
		}
		bool ReaderLockWhenWithTimeout(const Condition& cond, absl::Duration timeout) ABSL_SHARED_LOCK_FUNCTION() {
			return LockWhenCommon(cond, synchronization_internal::KernelTimeout{ timeout }, false);
		}
		...
		bool LockWhenWithDeadline(const Condition& cond, absl::Time deadline) ABSL_EXCLUSIVE_LOCK_FUNCTION() {
			return LockWhenCommon(cond, synchronization_internal::KernelTimeout{ deadline }, true);
		}
		bool ReaderLockWhenWithDeadline(const Condition& cond, absl::Time deadline) ABSL_SHARED_LOCK_FUNCTION() {
			return LockWhenCommon(cond, synchronization_internal::KernelTimeout{ deadline }, false);
		}
	  ...
	  private:
		...
		bool LockWhenCommon(const Condition& cond, synchronization_internal::KernelTimeout t, bool write);
		...
		void Fer(base_internal::PerThreadSynch* w);// used for CondVar->Mutex transfer
	...
	class ABSL_SCOPED_LOCKABLE MutexLock {
	  public:
		...
		explicit MutexLock(Mutex* mu, const Condition& cond) ABSL_EXCLUSIVE_LOCK_FUNCTION(mu) : mu_(mu) {
	...
	class ABSL_SCOPED_LOCKABLE ReaderMutexLock {
	  public:
		...
		explicit ReaderMutexLock(Mutex* mu, const Condition& cond) ABSL_SHARED_LOCK_FUNCTION(mu) : mu_(mu) {
	...
	class ABSL_SCOPED_LOCKABLE WriterMutexLock {
	  public:
		explicit WriterMutexLock(Mutex* mu) ABSL_EXCLUSIVE_LOCK_FUNCTION(mu) : mu_(mu) {
			mu->WriterLock();
		}

		explicit WriterMutexLock(Mutex* mu, const Condition& cond) ABSL_EXCLUSIVE_LOCK_FUNCTION(mu) : mu_(mu) {
	...
	class Condition {
	  public:
		...
		template<typename T> Condition(bool (*func)(T*), T* arg);
		...
		template<typename T, typename = void> Condition(bool (*func)(T*), typename absl::internal::type_identity<T>::type* arg);
		...
		template<typename T> Condition(T* object, bool (absl::internal::type_identity<T>::type::*method)());

		// Same as above, for const members
		template<typename T> Condition(const T* object, bool (absl::internal::type_identity<T>::type::*method)() const);
		...
		// `bool operator() const`.
		template<typename T, typename E = decltype(static_cast<bool (T::*)() const>(&T::operator()))> explicit Condition(const T* obj)
	  ...
	  private:
		...
		template<typename T, typename ConditionMethodPtr> static bool CastAndCallMethod(const Condition* c);

		// Helper methods for storing, validating, and reading callback arguments.
		template<typename T> inline void StoreCallback(T callback) {
			static_assert(sizeof(callback) <= sizeof(callback_), "An overlarge pointer was passed as a callback to Condition.");
			std::memcpy(callback_, &callback, sizeof(callback));
		}

		template<typename T> inline void ReadCallback(T* callback) const {
			std::memcpy(callback, callback_, sizeof(*callback));
		}

		static bool AlwaysTrue(const Condition*) {
			return true;
		}
	...
	class ABSL_SCOPED_LOCKABLE MutexLockMaybe {
	  public:
		explicit MutexLockMaybe(Mutex* mu) ABSL_EXCLUSIVE_LOCK_FUNCTION(mu) : mu_(mu) {
			if (this->mu_ != nullptr) {
				this->mu_->Lock();
			}
		}

		explicit MutexLockMaybe(Mutex* mu, const Condition& cond) ABSL_EXCLUSIVE_LOCK_FUNCTION(mu) : mu_(mu) {
			if (this->mu_ != nullptr) {
				this->mu_->LockWhen(cond);
			}
		}
	...
	class ABSL_SCOPED_LOCKABLE ReleasableMutexLock {
	  public:
		...
		explicit ReleasableMutexLock(Mutex* mu, const Condition& cond) ABSL_EXCLUSIVE_LOCK_FUNCTION(mu) : mu_(mu) {
			this->mu_->LockWhen(cond);
		}
	...
	// static
	template<typename T, typename ConditionMethodPtr> bool Condition::CastAndCallMethod(const Condition* c) {
	...
	// static
	template<typename T> bool Condition::CastAndCallFunction(const Condition* c) {
		bool (*function)(T*);
	...
	template<typename T> inline Condition::Condition(bool (*func)(T*), T* arg) : eval_(&CastAndCallFunction<T>), arg_(const_cast<void*>(static_cast<const void*>(arg))) {
		static_assert(sizeof(&func) <= sizeof(callback_), "An overlarge function pointer was passed to Condition.");
		StoreCallback(func);
	}

	template<typename T, typename> inline Condition::Condition(bool (*func)(T*), typename absl::internal::type_identity<T>::type* arg)
		// Just delegate to the overload above.
		: Condition(func, arg) {
	}

	template<typename T> inline Condition::Condition(T* object, bool (absl::internal::type_identity<T>::type::*method)())
		: eval_(&CastAndCallMethod<T, decltype(method)>), arg_(object) {
		static_assert(sizeof(&method) <= sizeof(callback_), "An overlarge method pointer was passed to Condition.");
		StoreCallback(method);
	}

	template<typename T> inline Condition::Condition(const T* object, bool (absl::internal::type_identity<T>::type::*method)() const)
		: eval_(&CastAndCallMethod<const T, decltype(method)>), arg_(reinterpret_cast<void*>(const_cast<T*>(object))) {
		StoreCallback(method);
	}
	...
	// measured by //../base/internal/cycleclock.h, and which may not
	...
	// //../base/internal/cycleclock.h, and which may not be real
	// "cycle" counts.)
	//
