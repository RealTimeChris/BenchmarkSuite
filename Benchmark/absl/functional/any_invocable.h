...
#include "../functional/internal/any_invocable.h"
...
namespace absl {
	...
	template<class Sig> class AnyInvocable : private internal_any_invocable::Impl<Sig> {
	  ...
	  public:
		...
		// empty.
		template<class F, typename = absl::enable_if_t<internal_any_invocable::CanConvert<Sig, F>::value>> AnyInvocable(F&& f)// NOLINT
		...
		explicit AnyInvocable(absl::in_place_type_t<T>, Args&&... args) : Impl(absl::in_place_type<absl::decay_t<T>>, std::forward<Args>(args)...) {
