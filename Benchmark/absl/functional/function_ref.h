...
#include "../functional/internal/function_ref.h"
...
namespace absl {
	...
	template<typename R, typename... Args> class FunctionRef<R(Args...)> {
	  private:
		...
		template<typename F, typename FR = absl::base_internal::invoke_result_t<F, Args&&...>> using EnableIfCompatible =
	  ...
	  public:
		...
		FunctionRef(const F& f ABSL_ATTRIBUTE_LIFETIME_BOUND) : invoker_(&absl::functional_internal::InvokeObject<F, R, Args...>) {
