...
namespace bnch_swt::internal {
	...
	template<typename event_count_t, uint64_t count_t> struct event_collector_type : public std::vector<event_count_t> {
		...
		template<typename function_type, typename... arg_types> BNCH_SWT_INLINE void run(function_type&& function, arg_types&&... args) {
			...
			std::vector<event_count_t>::operator[](currentIndex)
				.bytesProcessedVal.emplace(static_cast<uint64_t>(std::forward<function_type>(function)(std::forward<arg_types>(args)...)));
